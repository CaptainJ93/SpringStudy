...
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/delivery/scmComplaintHeader/business">配送包裹客诉处理header列表</a></li>
	</ul>
	<form:form id="searchForm" modelAttribute="scmComplaintHeader" action="${ctx}/delivery/scmComplaintHeader/business" method="post" class="breadcrumb form-search">
		<input id="pageNo" name="pageNo" type="hidden" value="${page.pageNo}"/>
		<input id="pageSize" name="pageSize" type="hidden" value="${page.pageSize}"/>
		<ul class="ul-form">
			
			<li><label>客诉类型:</label>
				<form:select path="cptype" class="input-medium">
					<form:option value="-1" label=""/>
					<form:options items="${fns:getDictList('CP_Type')}" itemLabel="label" itemValue="value" htmlEscape="false"/>
				</form:select>&nbsp;&nbsp;	
			</li>
			<li>
				<label>配送公司:</label><sys:treeselectCode id="cpselectedcarrier" name="cpselectedcarrier" value="${scmComplaintHeader.cpselectedcarrier}" labelName="carrier.carriername" labelValue="${scmComplaintHeader.carrier.carriername}"	title="配送商" url="/sys/office/treeData?type=4" cssClass="required"/>
			</li>
			<li><label>订单号:</label>

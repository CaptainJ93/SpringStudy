...
public class ScmSupplierCsoinfo extends DataEntity<ScmSupplierCsoinfo> {
	...
	@NotNull
	private Date createtimestart;//记录创建时间起
	@NotNull
	private Date createtimeend;//记录创建时间止
	...
	@Length(min = 0, max = 30, message = "客户身份证长度必须介于 0 和 30 之间")
	@ExcelField(title = "客户身份证", sort = 1410)
	...
	@ExcelField(title = "仓库指送", sort = 4000)
	...
	@ExcelField(title = "配送指示", sort = 4100)
	...
	@ExcelField(title = "通路", sort = 1010)
	@Length(min = 0, max = 7, message = "通路长度必须介于 0 和 7 之间")

...
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/deliveryquality/total">每日包裹整体监控</a></li>
	</ul>
	<shiro:hasPermission name="delivery:deliveryOrderintransit:view">
	<form:form id="searchForm" modelAttribute="deliveryQualityReportTotal" action="${ctx}/deliveryquality/total" method="post" class="breadcrumb form-search">
		<input id="pageNo" name="pageNo" type="hidden" value="${page.pageNo}"/>
		<input id="pageSize" name="pageSize" type="hidden" value="${page.pageSize}"/>
		
			<div>
			<label>配送公司:</label><sys:treeselectCode id="selectedcarrier" name="selectedcarrier" value="${deliveryQualityReportTotal.selectedcarrier}" labelName="carrier.carriername" labelValue="${deliveryQualityReportTotal.carrier.carriername}"	title="配送商" url="/sys/office/treeData?type=4" cssClass="required"/> 
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<c:forEach items="${page.list}" var="deliveryQualityReportTotal">
			<tr>
			<td>${deliveryQualityReportTotal.selectedcarrier}</td>
			<td>${deliveryQualityReportTotal.carrier.carriername}</td>

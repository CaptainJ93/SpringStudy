...
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/delivery/reportbycity/">报表城市列表</a></li>
	</ul>
	<shiro:hasPermission name="delivery:deliveryOrderintransit:view">
	<form:form id="searchForm" modelAttribute="deliveryQualityReportByCity" action="${ctx}/deliveryquality/reportbycity/" method="post" class="breadcrumb form-search">
		<input id="pageNo" name="pageNo" type="hidden" value="${page.pageNo}"/>
		<input id="pageSize" name="pageSize" type="hidden" value="${page.pageSize}"/>
		
			<div>
			<label>配送公司:</label><sys:treeselectCode id="selectedcarrier" name="selectedcarrier" value="${deliveryQualityReportByCity.selectedcarrier}" labelName="carrier.carriername" labelValue="${deliveryQualityReportByCity.carrier.carriername}"	title="配送商" url="/sys/office/treeData?type=4" cssClass="required"/> 
			<label>区域:</label><sys:treeselectCode id="areacode" name="areacode" value="${deliveryQualityReportByCity.areacode}" labelName="areainfo.type" labelValue="${deliveryQualityReportByCity.areainfo.type}"	title="区域" url="/sys/area/treeData" cssClass="required"/> <br />
			...
			<label>开始日期：</label><input id="createDateStart"  name="createDateStart"  type="text" readonly="readonly" maxlength="20" class="input-medium Wdate" style="width:163px;"
				value="<fmt:formatDate value="${deliveryQualityReportByCity.createDateStart}" pattern="yyyy-MM-dd HH:mm:ss"/>"
			...
			<label>结束日期：</label><input id="createDateEnd" name="createDateEnd" type="text" readonly="readonly" maxlength="20" class="input-medium Wdate" style="width:163px;"
				value="<fmt:formatDate value="${deliveryQualityReportByCity.createDateEnd}" pattern="yyyy-MM-dd HH:mm:ss"/>"
					onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'});"/>

...
public class ScmSupplierCsoinfoService extends CrudService<ScmSupplierCsoinfoDao, ScmSupplierCsoinfo> {
	...
	public void shipConfirm(List<ScmSupplierShipConfirmInfo> shipConfirmInfos) {
		String message = null;
		for (int i = 0; i < shipConfirmInfos.size(); i++) {
			ScmSupplierShipConfirmInfo shipConfirmInfo = shipConfirmInfos.get(i);
			if (shipConfirmInfo.getCarrierId() == null || shipConfirmInfo.getCarrierId() == 0) {
				message = String.format(errorMessageTemplate, "请填写配送公司代码，" + shipConfirmInfo);
				throw new ServiceException(message);
			}
			if (StringUtils.isEmpty(shipConfirmInfo.getTrackingNumber())) {
				message = String.format(errorMessageTemplate, "请填写配送单号，" + shipConfirmInfo);
				throw new ServiceException(message);
			}

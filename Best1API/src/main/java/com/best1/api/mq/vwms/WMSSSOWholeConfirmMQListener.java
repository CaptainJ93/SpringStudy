...
public class WMSSSOWholeConfirmMQListener implements ChannelAwareMessageListener{

	private static final Logger  logger =  LoggerFactory.getLogger(WMSSSOWholeConfirmMQListener.class); 
	@Autowired
	...
	public void onMessage(Message message, Channel channel) throws Exception {
		...
		try{
			...
			TvwsApiSuppliershipconfirm  tvwsApiSuppliershipconfirm =new TvwsApiSuppliershipconfirm();
			tvwsApiSuppliershipconfirm = JSON.parseObject(new String(message.getBody(),"utf-8"),TvwsApiSuppliershipconfirm.class);
			
			SuppliershipconfirmDTO dto = vwmsService.modifySupplierShipconfirm(tvwsApiSuppliershipconfirm);

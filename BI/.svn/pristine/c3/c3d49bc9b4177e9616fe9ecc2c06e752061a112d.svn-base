...
//		}
		...
		if($("#ageCustStatForm").find("#ageStart").val() != "" && ($("#ageCustStatForm").find("#ageEnd").val() == null || $("#ageCustStatForm").find("#ageEnd").val() == "")){
			$.messager.alert("操作提示", "起始年龄和终止年龄必须同时填写!");
			$("#ageCustStatForm").find("#ageEnd").focus();
			return false;
	 	}
		
		if($("#ageCustStatForm").find("#ageEnd").val() != "" && ($("#ageCustStatForm").find("#ageStart").val() == null || $("#ageCustStatForm").find("#ageStart").val() == "")){
			$.messager.alert("操作提示", "起始年龄和终止年龄必须同时填写!");
			$("#ageCustStatForm").find("#ageStart").focus();
			return false;
	 	}
		...
		if($("#registerEndTime").datebox("getValue") != "" &&($("#registerStartTime").datebox("getValue") == null || $("#registerStartTime").datebox("getValue") == "")){
			...
			$("#ageCustStatForm").find("#registerStartTime").focus();
			return false;
		}
		...
		if($("#actualEndTime").datebox("getValue") != "" &&($("#actualStartTime").datebox("getValue") == null || $("#actualStartTime").datebox("getValue") == "")){
			...
			$("#ageCustStatForm").find("#actualStartTime").focus();
			return false;
		}
		...
		if($("#orderEndTime").datebox("getValue") != "" &&($("#orderStartTime").datebox("getValue") == null || $("#orderStartTime").datebox("getValue") == "")){
			...
			$("#ageCustStatForm").find("#orderStartTime").focus();
			return false;
		}
		...
		jQuery("#ageCustStatTable").jqGrid("setGridParam", {url:"customerStat!ageCustStatList.action"}); 
		$("#ageCustStatTable").hlGridRefresh();
	});
	
	$("#resetBtn").click(function(){
		$("#ageCustStatForm").clearForm();
		//会员等级清空
		if($.fn.zTree.getZTreeObj("levelTree") != null){
			$.fn.zTree.getZTreeObj("levelTree").checkAllNodes(false);
		}
		//性别初始化
		$("#ageCustStatForm").find("#sex").val(null);
		$("#ageCustStatForm").find("#excludereturnflag").val("N");
		//会员类型
	...
	$("#clearFileCreatebyid").click(function() {
		...
		$("#clearLevel").click(function() {
			$("#ageCustStatForm").find("#membershiplevelname").val("");
			$("#ageCustStatForm").find("#membershiplevelid").val("");
		...
		$("#clearCustomerType").click(function() {
			$("#ageCustStatForm").find("#customerTypeName").val("");
			$("#ageCustStatForm").find("#customerTypeId").val("");
		...
		$("#clearProductClass").click(function() {
			$("#ageCustStatForm").find("#productClassName").val("");
			$("#ageCustStatForm").find("#productClassId").val("");
		...
		$("#clearSubchannel").click(function() {
			$("#ageCustStatForm").find("#subchannelName").val("");
			$("#ageCustStatForm").find("#subchannelId").val("");
		...
		$("#clearOrderType").click(function() {
			$("#ageCustStatForm").find("#orderTypeName").val("");
			$("#ageCustStatForm").find("#orderTypeId").val("");
		...
		$("#clearOrderStatus").click(function() {
			$("#ageCustStatForm").find("#orderStatusName").val("");
			$("#ageCustStatForm").find("#orderStatusId").val("");
...
function onCheckLvl(e, treeId, treeNode) {
	...
	$("#ageCustStatForm").find("#membershiplevelname").val(name);
	$("#ageCustStatForm").find("#membershiplevelid").val(id);
}
...
function onCheckCustomerType(e, treeId, treeNode) {
	...
	if (name.length > 0 ) name = name.substring(0, name.length-1);
	if (id.length > 0 ) id = id.substring(0, id.length-1);
	$("#ageCustStatForm").find("#customerTypeName").val(name);
	$("#ageCustStatForm").find("#customerTypeId").val(id);
}
...
function onCheckProductClass(e, treeId, treeNode) {
	...
	if (name.length > 0 ) name = name.substring(0, name.length-1);
	if (id.length > 0 ) id = id.substring(0, id.length-1);
	$("#ageCustStatForm").find("#productClassName").val(name);
	$("#ageCustStatForm").find("#productClassId").val(id);
}
...
function onCheckSubchannel(e, treeId, treeNode) {
	...
	if (name.length > 0 ) name = name.substring(0, name.length-1);
	if (id.length > 0 ) id = id.substring(0, id.length-1);
	$("#ageCustStatForm").find("#subchannelName").val(name);
	$("#ageCustStatForm").find("#subchannelId").val(id);
}
...
function onCheckOrderType(e, treeId, treeNode) {
	...
	if (name.length > 0 ) name = name.substring(0, name.length-1);
	if (id.length > 0 ) id = id.substring(0, id.length-1);
	$("#ageCustStatForm").find("#orderTypeName").val(name);
	$("#ageCustStatForm").find("#orderTypeId").val(id);
}
...
function onCheckOrderStatus(e, treeId, treeNode) {
	...
	if (name.length > 0 ) name = name.substring(0, name.length-1);
	if (id.length > 0 ) id = id.substring(0, id.length-1);
	$("#ageCustStatForm").find("#orderStatusName").val(name);
	$("#ageCustStatForm").find("#orderStatusId").val(id);
}
...
function ajaxFileUpload()
{
	...
	(
		{
			url:'customerStat!upload.action?paramMap.prefix=ageCustStat_product_',//用于文件上传的服务器端请求地址
...
function ajaxFileUploadUserId()
{
	...
	(
		{
			url:'customerStat!uploadUserId.action?paramMap.prefix=ageCustStat_userid_',//用于文件上传的服务器端请求地址
			secureuri:false,//一般设置为false

$(function(){
	$.getCaches("../cache!getCache.action","SEX,EXCLUDERETURNFLAG",function(cacheKeys){
		var options={
				...
				loadonce: false,
				caption:"查询列表",
				searchform: "#areaChannelStatForm",
				sortname:"city",
				allowOper : {add:false,del:false,update:false,exportexcel:true}
			};
		
		$("#areaChannelStatTable").hlGrid(options);
...
//		}
		...
		if($("#areaChannelStatForm").find("#ageStart").val() != "" && ($("#areaChannelStatForm").find("#ageEnd").val() == null || $("#areaChannelStatForm").find("#ageEnd").val() == "")){
			$.messager.alert("操作提示", "起始年龄和终止年龄必须同时填写!");
			$("#areaChannelStatForm").find("#ageEnd").focus();
			return false;
	 	}
		
		if($("#areaChannelStatForm").find("#ageEnd").val() != "" && ($("#areaChannelStatForm").find("#ageStart").val() == null || $("#areaChannelStatForm").find("#ageStart").val() == "")){
			$.messager.alert("操作提示", "起始年龄和终止年龄必须同时填写!");
			$("#areaChannelStatForm").find("#ageStart").focus();
			return false;
	 	}
		...
		if($("#registerEndTime").datebox("getValue") != "" &&($("#registerStartTime").datebox("getValue") == null || $("#registerStartTime").datebox("getValue") == "")){
			...
			$("#areaChannelStatForm").find("#registerStartTime").focus();
			return false;
		}
		...
		if($("#actualEndTime").datebox("getValue") != "" &&($("#actualStartTime").datebox("getValue") == null || $("#actualStartTime").datebox("getValue") == "")){
			...
			$("#areaChannelStatForm").find("#actualStartTime").focus();
			return false;
		}
		...
		if($("#orderEndTime").datebox("getValue") != "" &&($("#orderStartTime").datebox("getValue") == null || $("#orderStartTime").datebox("getValue") == "")){
			...
			$("#areaChannelStatForm").find("#orderStartTime").focus();
			return false;
		}
		...
		jQuery("#areaChannelStatTable").jqGrid("setGridParam", {url:"channelStat!areaChannelStatList.action"}); 
		$("#areaChannelStatTable").hlGridRefresh();
	});
	
	$("#resetBtn").click(function(){
		$("#areaChannelStatForm").clearForm();
		//地区清空
		$.fn.zTree.getZTreeObj("areaZipCodeTree").checkAllNodes(false);
		//会员等级清空
		$.fn.zTree.getZTreeObj("levelTree").checkAllNodes(false);
		//性别初始化
		$("#areaChannelStatForm").find("#sex").val(null);
		$("#areaChannelStatForm").find("#excludereturnflag").val("N");
		//会员类型
	...
	$("#clearFileCreatebyid").click(function() {
		...
		$("#clearLevel").click(function() {
			$("#areaChannelStatForm").find("#membershiplevelname").val("");
			$("#areaChannelStatForm").find("#membershiplevelid").val("");
		...
		$("#clearCustomerType").click(function() {
			$("#areaChannelStatForm").find("#customerTypeName").val("");
			$("#areaChannelStatForm").find("#customerTypeId").val("");
		...
		$("#clearProductClass").click(function() {
			$("#areaChannelStatForm").find("#productClassName").val("");
			$("#areaChannelStatForm").find("#productClassId").val("");
		...
		$("#clearSubchannel").click(function() {
			$("#areaChannelStatForm").find("#subchannelName").val("");
			$("#areaChannelStatForm").find("#subchannelId").val("");
		...
		$("#clearOrderType").click(function() {
			$("#areaChannelStatForm").find("#orderTypeName").val("");
			$("#areaChannelStatForm").find("#orderTypeId").val("");
		...
		$("#clearOrderStatus").click(function() {
			$("#areaChannelStatForm").find("#orderStatusName").val("");
			$("#areaChannelStatForm").find("#orderStatusId").val("");
		...
		var areaZipCode = {
				...
				async: {
					enable: true,
					url:"./query!queryCityCode.action",
				},
				callback: {
		...
		$("#clearAreaZipCode").click(function() {
			$("#areaChannelStatForm").find("#areaZipCodeName").val("");
			$("#areaChannelStatForm").find("#areaZipCodeId").val("");
...
function onCheckLvl(e, treeId, treeNode) {
	...
	$("#areaChannelStatForm").find("#membershiplevelname").val(name);
	$("#areaChannelStatForm").find("#membershiplevelid").val(id);
}
...
function onCheckCustomerType(e, treeId, treeNode) {
	...
	if (name.length > 0 ) name = name.substring(0, name.length-1);
	if (id.length > 0 ) id = id.substring(0, id.length-1);
	$("#areaChannelStatForm").find("#customerTypeName").val(name);
	$("#areaChannelStatForm").find("#customerTypeId").val(id);
}
...
function onCheckProductClass(e, treeId, treeNode) {
	...
	if (name.length > 0 ) name = name.substring(0, name.length-1);
	if (id.length > 0 ) id = id.substring(0, id.length-1);
	$("#areaChannelStatForm").find("#productClassName").val(name);
	$("#areaChannelStatForm").find("#productClassId").val(id);
}
...
function onCheckSubchannel(e, treeId, treeNode) {
	...
	if (name.length > 0 ) name = name.substring(0, name.length-1);
	if (id.length > 0 ) id = id.substring(0, id.length-1);
	$("#areaChannelStatForm").find("#subchannelName").val(name);
	$("#areaChannelStatForm").find("#subchannelId").val(id);
}
...
function onCheckOrderType(e, treeId, treeNode) {
	...
	if (name.length > 0 ) name = name.substring(0, name.length-1);
	if (id.length > 0 ) id = id.substring(0, id.length-1);
	$("#areaChannelStatForm").find("#orderTypeName").val(name);
	$("#areaChannelStatForm").find("#orderTypeId").val(id);
}
...
function onCheckOrderStatus(e, treeId, treeNode) {
	...
	if (name.length > 0 ) name = name.substring(0, name.length-1);
	if (id.length > 0 ) id = id.substring(0, id.length-1);
	$("#areaChannelStatForm").find("#orderStatusName").val(name);
	$("#areaChannelStatForm").find("#orderStatusId").val(id);
}
...
function onCheckAreaZipCode(e, treeId, treeNode) {
	...
	if (name.length > 0 ) name = name.substring(0, name.length-1);
	if (id.length > 0 ) id = id.substring(0, id.length-1);
	$("#areaChannelStatForm").find("#areaZipCodeName").val(name);
	$("#areaChannelStatForm").find("#areaZipCodeId").val(id);
}
...
function ajaxFileUpload()
{
	...
	(
		{
			url:'channelStat!upload.action?paramMap.prefix=areachannelstat_product_',//用于文件上传的服务器端请求地址
...
function ajaxFileUploadUserId()
{
	...
	(
		{
			url:'channelStat!uploadUserId.action?paramMap.prefix=areachannelstat_userid_',//用于文件上传的服务器端请求地址
			secureuri:false,//一般设置为false

$(function(){
	$.getCaches("../cache!getCache.action","SEX,EXCLUDERETURNFLAG",function(cacheKeys){
		var options={
				...
				sortname:"province,ordercount",
				sortorder:"desc",
				allowOper : {add:false,del:false,update:false,exportexcel:true}
			};
		
		$("#channelAreaStatTable").hlGrid(options);
...
//		}
		...
		if($("#channelAreaStatForm").find("#ageStart").val() != "" && ($("#channelAreaStatForm").find("#ageEnd").val() == null || $("#channelAreaStatForm").find("#ageEnd").val() == "")){
			$.messager.alert("操作提示", "起始年龄和终止年龄必须同时填写!");
			$("#channelAreaStatForm").find("#ageEnd").focus();
			return false;
	 	}
		
		if($("#channelAreaStatForm").find("#ageEnd").val() != "" && ($("#channelAreaStatForm").find("#ageStart").val() == null || $("#channelAreaStatForm").find("#ageStart").val() == "")){
			$.messager.alert("操作提示", "起始年龄和终止年龄必须同时填写!");
			$("#channelAreaStatForm").find("#ageStart").focus();
			return false;
	 	}
		...
		if($("#registerEndTime").datebox("getValue") != "" &&($("#registerStartTime").datebox("getValue") == null || $("#registerStartTime").datebox("getValue") == "")){
			...
			$("#channelAreaStatForm").find("#registerStartTime").focus();
			return false;
		}
		...
		if($("#actualEndTime").datebox("getValue") != "" &&($("#actualStartTime").datebox("getValue") == null || $("#actualStartTime").datebox("getValue") == "")){
			...
			$("#channelAreaStatForm").find("#actualStartTime").focus();
			return false;
		}
		...
		if($("#orderEndTime").datebox("getValue") != "" &&($("#orderStartTime").datebox("getValue") == null || $("#orderStartTime").datebox("getValue") == "")){
			...
			$("#channelAreaStatForm").find("#orderStartTime").focus();
			return false;
		}
		...
		jQuery("#channelAreaStatTable").jqGrid("setGridParam", {url:"channelStat!channelAreaStatList.action"}); 
		$("#channelAreaStatTable").hlGridRefresh();
	});
	
	$("#resetBtn").click(function(){
		$("#channelAreaStatForm").clearForm();
		//地区清空
		if($.fn.zTree.getZTreeObj("areaZipCodeTree") != null){
		...
		//性别初始化
		$("#channelAreaStatForm").find("#sex").val(null);
		$("#channelAreaStatForm").find("#excludereturnflag").val("N");
		//会员类型
	...
	$("#clearAreaZipCode").click(function() {
		$("#channelAreaStatForm").find("#areaZipCodeName").val("");
		$("#channelAreaStatForm").find("#areaZipCodeId").val("");
	 ...
	 */
		...
		$("#clearLevel").click(function() {
			$("#channelAreaStatForm").find("#membershiplevelname").val("");
			$("#channelAreaStatForm").find("#membershiplevelid").val("");
		...
		$("#clearCustomerType").click(function() {
			$("#channelAreaStatForm").find("#customerTypeName").val("");
			$("#channelAreaStatForm").find("#customerTypeId").val("");
		...
		$("#clearProductClass").click(function() {
			$("#channelAreaStatForm").find("#productClassName").val("");
			$("#channelAreaStatForm").find("#productClassId").val("");
		...
		$("#clearSubchannel").click(function() {
			$("#channelAreaStatForm").find("#subchannelName").val("");
			$("#channelAreaStatForm").find("#subchannelId").val("");
		...
		$("#clearOrderType").click(function() {
			$("#channelAreaStatForm").find("#orderTypeName").val("");
			$("#channelAreaStatForm").find("#orderTypeId").val("");
		...
		$("#clearOrderStatus").click(function() {
			$("#channelAreaStatForm").find("#orderStatusName").val("");
			$("#channelAreaStatForm").find("#orderStatusId").val("");
...
function onCheckAreaZipCode(e, treeId, treeNode) {
	...
	$("#channelAreaStatForm").find("#areaZipCodeName").val(name);
	$("#channelAreaStatForm").find("#areaZipCodeId").val(id);
}
...
function onCheckLvl(e, treeId, treeNode) {
	...
	if (name.length > 0 ) name = name.substring(0, name.length-1);
	if (id.length > 0 ) id = id.substring(0, id.length-1);
	$("#channelAreaStatForm").find("#membershiplevelname").val(name);
	$("#channelAreaStatForm").find("#membershiplevelid").val(id);
}
...
function onCheckCustomerType(e, treeId, treeNode) {
	...
	if (name.length > 0 ) name = name.substring(0, name.length-1);
	if (id.length > 0 ) id = id.substring(0, id.length-1);
	$("#channelAreaStatForm").find("#customerTypeName").val(name);
	$("#channelAreaStatForm").find("#customerTypeId").val(id);
}
...
function onCheckProductClass(e, treeId, treeNode) {
	...
	if (name.length > 0 ) name = name.substring(0, name.length-1);
	if (id.length > 0 ) id = id.substring(0, id.length-1);
	$("#channelAreaStatForm").find("#productClassName").val(name);
	$("#channelAreaStatForm").find("#productClassId").val(id);
}
...
function onCheckSubchannel(e, treeId, treeNode) {
	...
	if (name.length > 0 ) name = name.substring(0, name.length-1);
	if (id.length > 0 ) id = id.substring(0, id.length-1);
	$("#channelAreaStatForm").find("#subchannelName").val(name);
	$("#channelAreaStatForm").find("#subchannelId").val(id);
}
...
function onCheckOrderType(e, treeId, treeNode) {
	...
	if (name.length > 0 ) name = name.substring(0, name.length-1);
	if (id.length > 0 ) id = id.substring(0, id.length-1);
	$("#channelAreaStatForm").find("#orderTypeName").val(name);
	$("#channelAreaStatForm").find("#orderTypeId").val(id);
}
...
function onCheckOrderStatus(e, treeId, treeNode) {
	...
	if (name.length > 0 ) name = name.substring(0, name.length-1);
	if (id.length > 0 ) id = id.substring(0, id.length-1);
	$("#channelAreaStatForm").find("#orderStatusName").val(name);
	$("#channelAreaStatForm").find("#orderStatusId").val(id);
}
...
function ajaxFileUpload()
{
	...
	(
		{
			url:'channelStat!upload.action?paramMap.prefix=channelareastat_product_',//用于文件上传的服务器端请求地址
...
function ajaxFileUploadUserId()
{
	...
	(
		{
			url:'channelStat!uploadUserId.action?paramMap.prefix=channelareastat_userid_',//用于文件上传的服务器端请求地址
			secureuri:false,//一般设置为false

...
//add by ylw  add url attr to tree-node 
$.data(_cf[0],"tree-node",{id:_ce.id,text:_ce.text,url:_ce.url,iconCls:_ce.iconCls,attributes:_ce.attributes});
...
//add by ylw
var curl=_d5.url;
if ($(ul).attr("url")) curl=$(ul).attr("url");
_d3["nd"]=(new Date).getTime();
$.ajax({type:_d5.method,url:curl,data:_d3,dataType:"json",success:function(_d9){
_d8.removeClass("tree-loading");
//update by ylw
if (_d9!=undefined&&typeof _d9=="object"&&_d9["rows"])
_c2(_d2,ul,_d9.rows);
else
_c2(_d2,ul,_d9);
//update by ylw
...
_e0[0].url=_df.url; //add by ylw
_d1(_db,_e0[0],{"id":_df.id,"paramMap.id":_df.id},function(){
if(_de.animate){
...
$.fn.tree.defaults={url:null,method:"get",animate:false,checkbox:false,cascadeCheck:true,onlyLeafCheck:false,dnd:false,data:null,onBeforeLoad:function(node,_14e){
...
*/
$.each($("div,span,input,select,textarea,ul",_154),function(k,v){
	if(typeof($(v).attr("class"))=="undefined") return;
	var pos=$(v).attr("class").indexOf("easyui");
	if(pos<0)return;
	var name=$(v).attr("class").substring(pos+7,100);
	var pos=name.indexOf(" ");
	var name=name.substring(0,pos<0?100:pos);
	if(name=="select"||name=="input"||name=="cascadeselect")return;
	if($(v)[name])$(v)[name]();else aa.push({name:name,jq:r});
});//ylw
...
if ($.data(_179,"panel")==undefined) return; //ylw
...
//add by ylw
	if (_1d2=="open") {
		var urladdrs=window.location.href.split("#");
		window.location.href = urladdrs[0] + "#" + (new Date()).getTime();	
	}
//add by ylw
...
$.fn.dialog.defaults=$.extend({},$.fn.window.defaults,{title:"New Dialog",collapsible:false,minimizable:false,maximizable:false,resizable:false,toolbar:null,buttons:null,closed:true});
})(jQuery);
(function($){
...
//modify by ylw
var opts=$.extend({showType:"fade",showSpeed:600,showPosition:"center",width:250,height:60,msg:"",title:"",timeout:4000},typeof _1e3=="string"?{msg:_1e3}:_1e3||{});
//modify by ylw
var win=$("<div class=\"messager-body\"></div>").html(opts.msg).appendTo("body");
win.window({title:opts.title,width:opts.width,height:opts.height,collapsible:false,minimizable:false,maximizable:false,shadow:false,draggable:false,resizable:false,closed:true,
onBeforeOpen:function(){
...
if (opts.showPosition=="rightbottom")
...
if (opts.showPosition=="righttop")
	win.window("window").css({left:"",top:document.body.scrollTop,right:0,zIndex:$.fn.window.defaults.zIndex++});
win.window("open");
...
if(cc.attr("collapsed")=="false")//add by ylw
...
var t=$(_603);
return $.extend({},$.fn.validatebox.parseOptions(_603),{width:(parseInt(_603.style.width)||undefined),panelWidth:(parseInt(t.attr("panelWidth"))||undefined),panelHeight:(t.attr("panelHeight")=="auto"?"auto":parseInt(t.attr("panelHeight"))||undefined),separator:(t.attr("separator")||undefined),multiple:(t.attr("multiple")?(t.attr("multiple")=="true"||t.attr("multiple")==true):false),editable:(t.attr("editable")?t.attr("editable")=="true":undefined),disabled:(t.attr("disabled")?true:undefined),hasDownArrow:(t.attr("hasDownArrow")?t.attr("hasDownArrow")=="true":undefined),value:(t.val()||undefined)});
};
...
//add by ylw
if (typeof data.rows=="object") _623(_629,data.rows,_62b); else _623(_629,data,_62b);
//add by ylw
},error:function(){
...
if(jq.hasClass("jqueryui-datebox")){jq.val(_683);return;}//ylw compatible jqueryui datebox
return jq.each(function(){
_678(this,_683);
});
}
,getValue:function(jq){if(jq.hasClass("jqueryui-datebox"))return jq.val();else return jq.combo("getValue");} //ylw compatible jqueryui datebox
};

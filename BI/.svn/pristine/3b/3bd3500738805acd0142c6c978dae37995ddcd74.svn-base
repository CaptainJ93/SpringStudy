...
$.getCaches("../cache!getCache.action","",function(cacheKeys){
	var options={
			...
			sortorder:"desc",
			searchform: "#productComplaintForm",
			allowOper : {add:false,del:false,update:false,exportexcel:true}
		};
	$("#productComplaintTable").hlGrid(options);
...
//		}
		...
		if($("#orderEndTime").datebox("getValue") != "" &&($("#orderStartTime").datebox("getValue") == null || $("#orderStartTime").datebox("getValue") == "")){
			...
			$("#productComplaintForm").find("#orderStartTime").focus();
			return false;
		}
		...
		jQuery("#productComplaintTable").jqGrid("setGridParam", {url:"customerStat!productComplaintList.action"}); 
		$("#productComplaintTable").hlGridRefresh();
	});
	
	$("#resetBtn").click(function(){
		$("#productComplaintForm").clearForm();
		//会员等级清空
	...
	$("#clearFileHandlebyid").click(function() {
		...
		$("#clearLevel").click(function() {
			$("#productComplaintForm").find("#membershiplevelname").val("");
			$("#productComplaintForm").find("#membershiplevelid").val("");
		...
		$("#clearCustomerType").click(function() {
			$("#productComplaintForm").find("#customerTypeName").val("");
			$("#productComplaintForm").find("#customerTypeId").val("");
		...
		$("#clearReturnReason").click(function() {
			$("#productComplaintForm").find("#returnReasonName").val("");
			$("#productComplaintForm").find("#returnReasonId").val("");
		...
		$("#clearComplaintCategory").click(function() {
			$("#productComplaintForm").find("#complaintCategoryName").val("");
			$("#productComplaintForm").find("#complaintCategoryId").val("");
...
function onCheckLvl(e, treeId, treeNode) {
	...
	$("#productComplaintForm").find("#membershiplevelname").val(name);
	$("#productComplaintForm").find("#membershiplevelid").val(id);
}
...
function onCheckCustomerType(e, treeId, treeNode) {
	...
	if (name.length > 0 ) name = name.substring(0, name.length-1);
	if (id.length > 0 ) id = id.substring(0, id.length-1);
	$("#productComplaintForm").find("#customerTypeName").val(name);
	$("#productComplaintForm").find("#customerTypeId").val(id);
}
...
function onCheckReturnReason(e, treeId, treeNode) {
	...
	if (name.length > 0 ) name = name.substring(0, name.length-1);
	if (id.length > 0 ) id = id.substring(0, id.length-1);
	$("#productComplaintForm").find("#returnReasonName").val(name);
	$("#productComplaintForm").find("#returnReasonId").val(id);
}
...
function onCheckComplaintCategory(e, treeId, treeNode) {
	...
	if (name.length > 0 ) name = name.substring(0, name.length-1);
	if (id.length > 0 ) id = id.substring(0, id.length-1);
	$("#productComplaintForm").find("#complaintCategoryName").val(name);
	$("#productComplaintForm").find("#complaintCategoryId").val(id);
}
...
function ajaxFileUpload()
{
	...
	(
		{
			url:'customerStat!upload.action?paramMap.prefix=productComplaint_product_',//用于文件上传的服务器端请求地址
...
function ajaxFileUploadUserId()
{
	...
	(
		{
			url:'customerStat!uploadUserId.action?paramMap.prefix=productComplaint_userid_',//用于文件上传的服务器端请求地址
			secureuri:false,//一般设置为false
...
function ajaxFileUploadUserId2()
{
	...
	(
		{
			url:'customerStat!uploadHandleUserId.action?paramMap.prefix=productComplaint_userid2_',//用于文件上传的服务器端请求地址
			secureuri:false,//一般设置为false

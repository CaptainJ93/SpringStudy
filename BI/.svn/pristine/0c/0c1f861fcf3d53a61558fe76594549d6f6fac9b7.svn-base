...
public class AssessStatAction extends BaseAction {
	private AssessStatService assessStatService;
	private File file;
	...
	private File fileMDid;
	private String fileMDidFileName;
	
	private String excludereturnflag;

	public String mdOrderStatList() throws Exception {
		DataModel resData = assessStatService.getMdOrderStatByPage(paramMap);
		//判断是否是excel导出
	...
	public String createbyStatList() throws Exception {
		DataModel resData = assessStatService.getCreatebyStatByPage(paramMap);
		//判断是否是excel导出
		if(paramMap.get("export")!=null &&"excel".equals(paramMap.get("export"))){
			dealGarbled();//处理paramMap的乱码问题
			commonOutExcel(resData);
	 ...
	 * @throws Exception
	 */
	public String uploadMDId() throws Exception {
		DataModel resData = new DataModel();
		String path = ServletActionContext.getRequest().getRealPath(ReportCommonDefine.ASSESSSTAT_FILEPATH);
		try {
			File f = this.getFileMDid();
			if(!this.getFileMDidFileName().toLowerCase().endsWith(".xls")){
				resData.setErrCode("1");
				resData.setErrMsg("请选择2003版excel文件进行上传！");
				commonOutPrintForFileUpload(resData);
				return null;
			...
			FileInputStream inputStream = new FileInputStream(f);
			UserInfo user = (UserInfo) getSession().getAttribute("user");
			String fileName = paramMap.get("prefix").toString()+user.getUserid()+this.getFileMDidFileName().substring(this.getFileMDidFileName().lastIndexOf("."),this.getFileMDidFileName().length());
			FileOutputStream outputStream = new FileOutputStream(path + "/" + fileName);
			byte[] buf = new byte[1024];
			int length = 0;
			while ((length = inputStream.read(buf)) != -1) {
			...
			//解析文件内容是否满足要求
			resData = checkFile(path + "/" + fileName);
			resData.setValue(subFileName(this.getFileMDidFileName())+","+fileName);
		} catch (Exception e) {
			e.printStackTrace();
			resData.setErrCode("1");
			resData.setErrMsg("对不起,文件上传失败了!");
	...
	public AssessStatService getAssessStatService() {
		return assessStatService;
	}

	public void setAssessStatService(AssessStatService assessStatService) {
		this.assessStatService = assessStatService;
	}
	...
	public File getFileMDid() {
		return fileMDid;
	}

	public void setFileMDid(File fileMDid) {
		this.fileMDid = fileMDid;
	}

	public String getFileMDidFileName() {
		return fileMDidFileName;
	}

	public void setFileMDidFileName(String fileMDidFileName) {
		this.fileMDidFileName = fileMDidFileName;
	}

$(function(){
	$.getCaches("../cache!getCache.action","SEX,EXCLUDERETURNFLAG",function(cacheKeys){
		var options={
				...
				sortname:"quantity",
				sortorder:"desc",
				searchform: "#buytimeCustStatForm",
				allowOper : {add:false,del:false,update:false,exportexcel:true}
			};
		$("#buytimeCustStatTable").hlGrid(options);
...
//		}
		...
		if($("#buytimeCustStatForm").find("#ageStart").val() != "" && ($("#buytimeCustStatForm").find("#ageEnd").val() == null || $("#buytimeCustStatForm").find("#ageEnd").val() == "")){
			$.messager.alert("操作提示", "起始年龄和终止年龄必须同时填写!");
			$("#buytimeCustStatForm").find("#ageEnd").focus();
			return false;
	 	}
		
		if($("#buytimeCustStatForm").find("#ageEnd").val() != "" && ($("#buytimeCustStatForm").find("#ageStart").val() == null || $("#buytimeCustStatForm").find("#ageStart").val() == "")){
			$.messager.alert("操作提示", "起始年龄和终止年龄必须同时填写!");
			$("#buytimeCustStatForm").find("#ageStart").focus();
			return false;
	 	}
		...
		if($("#registerEndTime").datebox("getValue") != "" &&($("#registerStartTime").datebox("getValue") == null || $("#registerStartTime").datebox("getValue") == "")){
			...
			$("#buytimeCustStatForm").find("#registerStartTime").focus();
			return false;
		}
		...
		if($("#actualEndTime").datebox("getValue") != "" &&($("#actualStartTime").datebox("getValue") == null || $("#actualStartTime").datebox("getValue") == "")){
			...
			$("#buytimeCustStatForm").find("#actualStartTime").focus();
			return false;
		}
		...
		if($("#orderEndTime").datebox("getValue") != "" &&($("#orderStartTime").datebox("getValue") == null || $("#orderStartTime").datebox("getValue") == "")){
			...
			$("#buytimeCustStatForm").find("#orderStartTime").focus();
			return false;
		}
		...
		jQuery("#buytimeCustStatTable").jqGrid("setGridParam", {url:"customerStat!buytimeCustStatList.action"}); 
		$("#buytimeCustStatTable").hlGridRefresh();
	});
	
	$("#resetBtn").click(function(){
		$("#buytimeCustStatForm").clearForm();
		//会员等级清空
		if($.fn.zTree.getZTreeObj("levelTree") != null){
			$.fn.zTree.getZTreeObj("levelTree").checkAllNodes(false);
		}
		//性别初始化
		$("#buytimeCustStatForm").find("#sex").val(null);
		$("#buytimeCustStatForm").find("#excludereturnflag").val("N");
		//会员类型
	...
	$("#clearFileCreatebyid").click(function() {
		...
		$("#clearLevel").click(function() {
			$("#buytimeCustStatForm").find("#membershiplevelname").val("");
			$("#buytimeCustStatForm").find("#membershiplevelid").val("");
		...
		$("#clearCustomerType").click(function() {
			$("#buytimeCustStatForm").find("#customerTypeName").val("");
			$("#buytimeCustStatForm").find("#customerTypeId").val("");
		...
		$("#clearProductClass").click(function() {
			$("#buytimeCustStatForm").find("#productClassName").val("");
			$("#buytimeCustStatForm").find("#productClassId").val("");
		...
		$("#clearSubchannel").click(function() {
			$("#buytimeCustStatForm").find("#subchannelName").val("");
			$("#buytimeCustStatForm").find("#subchannelId").val("");
		...
		$("#clearOrderType").click(function() {
			$("#buytimeCustStatForm").find("#orderTypeName").val("");
			$("#buytimeCustStatForm").find("#orderTypeId").val("");
		...
		$("#clearOrderStatus").click(function() {
			$("#buytimeCustStatForm").find("#orderStatusName").val("");
			$("#buytimeCustStatForm").find("#orderStatusId").val("");
...
function onCheckLvl(e, treeId, treeNode) {
	...
	$("#buytimeCustStatForm").find("#membershiplevelname").val(name);
	$("#buytimeCustStatForm").find("#membershiplevelid").val(id);
}
...
function onCheckCustomerType(e, treeId, treeNode) {
	...
	if (name.length > 0 ) name = name.substring(0, name.length-1);
	if (id.length > 0 ) id = id.substring(0, id.length-1);
	$("#buytimeCustStatForm").find("#customerTypeName").val(name);
	$("#buytimeCustStatForm").find("#customerTypeId").val(id);
}
...
function onCheckProductClass(e, treeId, treeNode) {
	...
	if (name.length > 0 ) name = name.substring(0, name.length-1);
	if (id.length > 0 ) id = id.substring(0, id.length-1);
	$("#buytimeCustStatForm").find("#productClassName").val(name);
	$("#buytimeCustStatForm").find("#productClassId").val(id);
}
...
function onCheckSubchannel(e, treeId, treeNode) {
	...
	if (name.length > 0 ) name = name.substring(0, name.length-1);
	if (id.length > 0 ) id = id.substring(0, id.length-1);
	$("#buytimeCustStatForm").find("#subchannelName").val(name);
	$("#buytimeCustStatForm").find("#subchannelId").val(id);
}
...
function onCheckOrderType(e, treeId, treeNode) {
	...
	if (name.length > 0 ) name = name.substring(0, name.length-1);
	if (id.length > 0 ) id = id.substring(0, id.length-1);
	$("#buytimeCustStatForm").find("#orderTypeName").val(name);
	$("#buytimeCustStatForm").find("#orderTypeId").val(id);
}
...
function onCheckOrderStatus(e, treeId, treeNode) {
	...
	if (name.length > 0 ) name = name.substring(0, name.length-1);
	if (id.length > 0 ) id = id.substring(0, id.length-1);
	$("#buytimeCustStatForm").find("#orderStatusName").val(name);
	$("#buytimeCustStatForm").find("#orderStatusId").val(id);
}
...
function ajaxFileUpload()
{
	...
	(
		{
			url:'customerStat!upload.action?paramMap.prefix=buytimeCustStat_product_',//用于文件上传的服务器端请求地址
...
function ajaxFileUploadUserId()
{
	...
	(
		{
			url:'customerStat!uploadUserId.action?paramMap.prefix=buytimeCustStat_userid_',//用于文件上传的服务器端请求地址
			secureuri:false,//一般设置为false

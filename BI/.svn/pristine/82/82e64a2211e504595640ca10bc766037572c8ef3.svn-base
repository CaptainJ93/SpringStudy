$(function(){
	$.getCaches("../cache!getCache.action","SEX,EXCLUDERETURNFLAG",function(cacheKeys){
		var options={
				...
				defaultSearch:false,
				colModel:[
				    {label:"编号",name:"createdby"},
				    {label:"名称",name:"createdbyname"},
				    {label:"接待会员数",name:"customercount"},
				...
				sortorder:"desc",
				searchform: "#createbyCustStatForm",
				allowOper : {add:false,del:false,update:false,exportexcel:true}
			};
		$("#createbyCustStatTable").hlGrid(options);
...
//		}
		...
		if($("#createbyCustStatForm").find("#ageStart").val() != "" && ($("#createbyCustStatForm").find("#ageEnd").val() == null || $("#createbyCustStatForm").find("#ageEnd").val() == "")){
			$.messager.alert("操作提示", "起始年龄和终止年龄必须同时填写!");
			$("#createbyCustStatForm").find("#ageEnd").focus();
			return false;
	 	}
		
		if($("#createbyCustStatForm").find("#ageEnd").val() != "" && ($("#createbyCustStatForm").find("#ageStart").val() == null || $("#createbyCustStatForm").find("#ageStart").val() == "")){
			$.messager.alert("操作提示", "起始年龄和终止年龄必须同时填写!");
			$("#createbyCustStatForm").find("#ageStart").focus();
			return false;
	 	}
		...
		if($("#registerEndTime").datebox("getValue") != "" &&($("#registerStartTime").datebox("getValue") == null || $("#registerStartTime").datebox("getValue") == "")){
			...
			$("#createbyCustStatForm").find("#registerStartTime").focus();
			return false;
		}
		...
		if($("#actualEndTime").datebox("getValue") != "" &&($("#actualStartTime").datebox("getValue") == null || $("#actualStartTime").datebox("getValue") == "")){
			...
			$("#createbyCustStatForm").find("#actualStartTime").focus();
			return false;
		}
		...
		if($("#orderEndTime").datebox("getValue") != "" &&($("#orderStartTime").datebox("getValue") == null || $("#orderStartTime").datebox("getValue") == "")){
			...
			$("#createbyCustStatForm").find("#orderStartTime").focus();
			return false;
		}
		...
		jQuery("#createbyCustStatTable").jqGrid("setGridParam", {url:"customerStat!createbyCustStatList.action"}); 
		$("#createbyCustStatTable").hlGridRefresh();
	});
	
	$("#resetBtn").click(function(){
		$("#createbyCustStatForm").clearForm();
		//会员等级清空
		if($.fn.zTree.getZTreeObj("levelTree") != null){
			$.fn.zTree.getZTreeObj("levelTree").checkAllNodes(false);
		}
		//性别初始化
		$("#createbyCustStatForm").find("#sex").val(null);
		$("#createbyCustStatForm").find("#excludereturnflag").val("N");
		//会员类型
	...
	$("#clearFileCreatebyid").click(function() {
		...
		$("#clearLevel").click(function() {
			$("#createbyCustStatForm").find("#membershiplevelname").val("");
			$("#createbyCustStatForm").find("#membershiplevelid").val("");
		...
		$("#clearCustomerType").click(function() {
			$("#createbyCustStatForm").find("#customerTypeName").val("");
			$("#createbyCustStatForm").find("#customerTypeId").val("");
		...
		$("#clearProductClass").click(function() {
			$("#createbyCustStatForm").find("#productClassName").val("");
			$("#createbyCustStatForm").find("#productClassId").val("");
		...
		$("#clearSubchannel").click(function() {
			$("#createbyCustStatForm").find("#subchannelName").val("");
			$("#createbyCustStatForm").find("#subchannelId").val("");
		...
		$("#clearOrderType").click(function() {
			$("#createbyCustStatForm").find("#orderTypeName").val("");
			$("#createbyCustStatForm").find("#orderTypeId").val("");
		...
		$("#clearOrderStatus").click(function() {
			$("#createbyCustStatForm").find("#orderStatusName").val("");
			$("#createbyCustStatForm").find("#orderStatusId").val("");
...
function onCheckLvl(e, treeId, treeNode) {
	...
	$("#createbyCustStatForm").find("#membershiplevelname").val(name);
	$("#createbyCustStatForm").find("#membershiplevelid").val(id);
}
...
function onCheckCustomerType(e, treeId, treeNode) {
	...
	if (name.length > 0 ) name = name.substring(0, name.length-1);
	if (id.length > 0 ) id = id.substring(0, id.length-1);
	$("#createbyCustStatForm").find("#customerTypeName").val(name);
	$("#createbyCustStatForm").find("#customerTypeId").val(id);
}
...
function onCheckProductClass(e, treeId, treeNode) {
	...
	if (name.length > 0 ) name = name.substring(0, name.length-1);
	if (id.length > 0 ) id = id.substring(0, id.length-1);
	$("#createbyCustStatForm").find("#productClassName").val(name);
	$("#createbyCustStatForm").find("#productClassId").val(id);
}
...
function onCheckSubchannel(e, treeId, treeNode) {
	...
	if (name.length > 0 ) name = name.substring(0, name.length-1);
	if (id.length > 0 ) id = id.substring(0, id.length-1);
	$("#createbyCustStatForm").find("#subchannelName").val(name);
	$("#createbyCustStatForm").find("#subchannelId").val(id);
}
...
function onCheckOrderType(e, treeId, treeNode) {
	...
	if (name.length > 0 ) name = name.substring(0, name.length-1);
	if (id.length > 0 ) id = id.substring(0, id.length-1);
	$("#createbyCustStatForm").find("#orderTypeName").val(name);
	$("#createbyCustStatForm").find("#orderTypeId").val(id);
}
...
function onCheckOrderStatus(e, treeId, treeNode) {
	...
	if (name.length > 0 ) name = name.substring(0, name.length-1);
	if (id.length > 0 ) id = id.substring(0, id.length-1);
	$("#createbyCustStatForm").find("#orderStatusName").val(name);
	$("#createbyCustStatForm").find("#orderStatusId").val(id);
}
...
function ajaxFileUpload()
{
	...
	(
		{
			url:'customerStat!upload.action?paramMap.prefix=createbyCustStat_product_',//用于文件上传的服务器端请求地址
...
function ajaxFileUploadUserId()
{
	...
	(
		{
			url:'customerStat!uploadUserId.action?paramMap.prefix=createbyCustStat_userid_',//用于文件上传的服务器端请求地址
			secureuri:false,//一般设置为false

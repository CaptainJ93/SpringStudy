$(function(){
	$.getCaches("../cache!getCache.action","SEX,EXCLUDERETURNFLAG",function(cacheKeys){
		var options={
				...
				defaultSearch:false,
				colModel:[
				    {label:"会员等级",name:"membershiplevelname"},
				    {label:"会员数",name:"customercount"},
				    {label:"占比",name:"pct_total"},
				    {label:"销售金额",name:"totalamount"},
				    {label:"毛利",name:"totalmaoli"}
				],
				multiselect:false,
				loadonce: false,
				caption:"查询列表",
				...
				sortorder:"desc",
				searchform: "#memberLevelCustStatForm",
		...
		$("#memberLevelCustStatTable").hlGrid(options);
		
		$("#orderStartTime").datebox("setValue",getDateString(-30)+" 00:00:00");
		$("#orderEndTime").datebox("setValue",getDateString(1)+" 00:00:00");
		
		var chart1 = new FusionCharts(contextPath+"/FusionChart/Pie3D.swf", "chartLevle", "600", "300");
		chart1.setDataXML("");
...
//		}
		...
		if($("#memberLevelCustStatForm").find("#ageStart").val() != "" && ($("#memberLevelCustStatForm").find("#ageEnd").val() == null || $("#memberLevelCustStatForm").find("#ageEnd").val() == "")){
			$.messager.alert("操作提示", "起始年龄和终止年龄必须同时填写!");
			$("#memberLevelCustStatForm").find("#ageEnd").focus();
			return false;
	 	}
		
		if($("#memberLevelCustStatForm").find("#ageEnd").val() != "" && ($("#memberLevelCustStatForm").find("#ageStart").val() == null || $("#memberLevelCustStatForm").find("#ageStart").val() == "")){
			$.messager.alert("操作提示", "起始年龄和终止年龄必须同时填写!");
			$("#memberLevelCustStatForm").find("#ageStart").focus();
			return false;
	 	}
		...
		if($("#registerEndTime").datebox("getValue") != "" &&($("#registerStartTime").datebox("getValue") == null || $("#registerStartTime").datebox("getValue") == "")){
			...
			$("#memberLevelCustStatForm").find("#registerStartTime").focus();
			return false;
		}
		...
		if($("#actualEndTime").datebox("getValue") != "" &&($("#actualStartTime").datebox("getValue") == null || $("#actualStartTime").datebox("getValue") == "")){
			...
			$("#memberLevelCustStatForm").find("#actualStartTime").focus();
			return false;
		}
		...
		if($("#orderEndTime").datebox("getValue") != "" &&($("#orderStartTime").datebox("getValue") == null || $("#orderStartTime").datebox("getValue") == "")){
			...
			$("#memberLevelCustStatForm").find("#orderStartTime").focus();
			return false;
		}
		...
		jQuery("#memberLevelCustStatTable").jqGrid("setGridParam", {url:"customerStat!memberLevelCustStatList.action"}); 
		$("#memberLevelCustStatTable").hlGridRefresh();
	});
	
	$("#resetBtn").click(function(){
		$("#memberLevelCustStatForm").clearForm();
		//会员等级清空
		if($.fn.zTree.getZTreeObj("levelTree") != null){
			$.fn.zTree.getZTreeObj("levelTree").checkAllNodes(false);
		}
		//性别初始化
		$("#memberLevelCustStatForm").find("#sex").val(null);
		$("#memberLevelCustStatForm").find("#excludereturnflag").val("N");
		//会员类型
	...
	$("#clearFileCreatebyid").click(function() {
		...
		$("#clearLevel").click(function() {
			$("#memberLevelCustStatForm").find("#membershiplevelname").val("");
			$("#memberLevelCustStatForm").find("#membershiplevelid").val("");
		...
		$("#clearCustomerType").click(function() {
			$("#memberLevelCustStatForm").find("#customerTypeName").val("");
			$("#memberLevelCustStatForm").find("#customerTypeId").val("");
		...
		$("#clearProductClass").click(function() {
			$("#memberLevelCustStatForm").find("#productClassName").val("");
			$("#memberLevelCustStatForm").find("#productClassId").val("");
		...
		$("#clearSubchannel").click(function() {
			$("#memberLevelCustStatForm").find("#subchannelName").val("");
			$("#memberLevelCustStatForm").find("#subchannelId").val("");
		...
		$("#clearOrderType").click(function() {
			$("#memberLevelCustStatForm").find("#orderTypeName").val("");
			$("#memberLevelCustStatForm").find("#orderTypeId").val("");
		...
		$("#clearOrderStatus").click(function() {
			$("#memberLevelCustStatForm").find("#orderStatusName").val("");
			$("#memberLevelCustStatForm").find("#orderStatusId").val("");
...
function onCheckLvl(e, treeId, treeNode) {
	...
	$("#memberLevelCustStatForm").find("#membershiplevelname").val(name);
	$("#memberLevelCustStatForm").find("#membershiplevelid").val(id);
}
...
function onCheckCustomerType(e, treeId, treeNode) {
	...
	if (name.length > 0 ) name = name.substring(0, name.length-1);
	if (id.length > 0 ) id = id.substring(0, id.length-1);
	$("#memberLevelCustStatForm").find("#customerTypeName").val(name);
	$("#memberLevelCustStatForm").find("#customerTypeId").val(id);
}
...
function onCheckProductClass(e, treeId, treeNode) {
	...
	if (name.length > 0 ) name = name.substring(0, name.length-1);
	if (id.length > 0 ) id = id.substring(0, id.length-1);
	$("#memberLevelCustStatForm").find("#productClassName").val(name);
	$("#memberLevelCustStatForm").find("#productClassId").val(id);
}
...
function onCheckSubchannel(e, treeId, treeNode) {
	...
	if (name.length > 0 ) name = name.substring(0, name.length-1);
	if (id.length > 0 ) id = id.substring(0, id.length-1);
	$("#memberLevelCustStatForm").find("#subchannelName").val(name);
	$("#memberLevelCustStatForm").find("#subchannelId").val(id);
}
...
function onCheckOrderType(e, treeId, treeNode) {
	...
	if (name.length > 0 ) name = name.substring(0, name.length-1);
	if (id.length > 0 ) id = id.substring(0, id.length-1);
	$("#memberLevelCustStatForm").find("#orderTypeName").val(name);
	$("#memberLevelCustStatForm").find("#orderTypeId").val(id);
}
...
function onCheckOrderStatus(e, treeId, treeNode) {
	...
	if (name.length > 0 ) name = name.substring(0, name.length-1);
	if (id.length > 0 ) id = id.substring(0, id.length-1);
	$("#memberLevelCustStatForm").find("#orderStatusName").val(name);
	$("#memberLevelCustStatForm").find("#orderStatusId").val(id);
}
...
function ajaxFileUpload()
{
	...
	(
		{
			url:'customerStat!upload.action?paramMap.prefix=memberLevelCustStat_product_',//用于文件上传的服务器端请求地址
...
function ajaxFileUploadUserId()
{
	...
	(
		{
			url:'customerStat!uploadUserId.action?paramMap.prefix=memberLevelCustStat_userid_',//用于文件上传的服务器端请求地址
			secureuri:false,//一般设置为false
...
function drawFusionChart(xml){
	if(getChartFromId("chartLevle")){
		var chartReference = FusionCharts("chartLevle");
		if(xml == null || xml.memberLevelCustStatChart == null || xml.memberLevelCustStatChart == ""){
			chartReference.setDataXML("");
		}else{
			chartReference.setDataXML(xml.memberLevelCustStatChart);
		}
	}else{
		...
		if(xml == null || xml.memberLevelCustStatChart == null || xml.memberLevelCustStatChart == ""){
			...
			chart1.setDataXML(xml.memberLevelCustStatChart);
		}
	　　 chart1.render("chart1Div");
	}
}

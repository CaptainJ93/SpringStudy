	...
	$.getCaches("../cache!getCache.action","SEX,EXCLUDERETURNFLAG",function(cacheKeys){
		var options={
				...
				colModel:[
				    ...
				    {label:"换货件数",name:"changecount"},
				    {label:"换货金额",name:"changeamount"},
				    {label:"销退件数",name:"returncount"},
...
//		}
		...
		if($("#mdOrderStatForm").find("#ageStart").val() != "" && ($("#mdOrderStatForm").find("#ageEnd").val() == null || $("#mdOrderStatForm").find("#ageEnd").val() == "")){
			$.messager.alert("操作提示", "起始年龄和终止年龄必须同时填写!");
			$("#mdOrderStatForm").find("#ageEnd").focus();
			return false;
	 	}
		
		if($("#mdOrderStatForm").find("#ageEnd").val() != "" && ($("#mdOrderStatForm").find("#ageStart").val() == null || $("#mdOrderStatForm").find("#ageStart").val() == "")){
			$.messager.alert("操作提示", "起始年龄和终止年龄必须同时填写!");
			$("#mdOrderStatForm").find("#ageStart").focus();
			return false;
	 	}
		...
		if($("#registerEndTime").datebox("getValue") != "" &&($("#registerStartTime").datebox("getValue") == null || $("#registerStartTime").datebox("getValue") == "")){
			...
			$("#mdOrderStatForm").find("#registerStartTime").focus();
			return false;
		}
		...
		if($("#actualEndTime").datebox("getValue") != "" &&($("#actualStartTime").datebox("getValue") == null || $("#actualStartTime").datebox("getValue") == "")){
			...
			$("#mdOrderStatForm").find("#actualStartTime").focus();
			return false;
		}
		...
		if($("#orderEndTime").datebox("getValue") != "" &&($("#orderStartTime").datebox("getValue") == null || $("#orderStartTime").datebox("getValue") == "")){
			...
			$("#mdOrderStatForm").find("#orderStartTime").focus();
			return false;
		}
		...
		jQuery("#mdOrderStatTable").jqGrid("setGridParam", {url:"assessStat!mdOrderStatList.action"}); 
		$("#mdOrderStatTable").hlGridRefresh();
	});
	
	$("#resetBtn").click(function(){
		$("#mdOrderStatForm").clearForm();
		//会员等级清空
		$.fn.zTree.getZTreeObj("levelTree").checkAllNodes(false);
		//性别初始化
		$("#mdOrderStatForm").find("#sex").val(null);
		$("#mdOrderStatForm").find("#excludereturnflag").val("N");
		//会员类型
	...
	$("#clearFileCreatebyid").click(function() {
		...
		$("#clearLevel").click(function() {
			$("#mdOrderStatForm").find("#membershiplevelname").val("");
			$("#mdOrderStatForm").find("#membershiplevelid").val("");
		...
		$("#clearCustomerType").click(function() {
			$("#mdOrderStatForm").find("#customerTypeName").val("");
			$("#mdOrderStatForm").find("#customerTypeId").val("");
		...
		$("#clearProductClass").click(function() {
			$("#mdOrderStatForm").find("#productClassName").val("");
			$("#mdOrderStatForm").find("#productClassId").val("");
		...
		$("#clearSubchannel").click(function() {
			$("#mdOrderStatForm").find("#subchannelName").val("");
			$("#mdOrderStatForm").find("#subchannelId").val("");
		...
		$("#clearOrderType").click(function() {
			$("#mdOrderStatForm").find("#orderTypeName").val("");
			$("#mdOrderStatForm").find("#orderTypeId").val("");
		...
		$("#clearOrderStatus").click(function() {
			$("#mdOrderStatForm").find("#orderStatusName").val("");
			$("#mdOrderStatForm").find("#orderStatusId").val("");
...
function onCheckLvl(e, treeId, treeNode) {
	...
	$("#mdOrderStatForm").find("#membershiplevelname").val(name);
	$("#mdOrderStatForm").find("#membershiplevelid").val(id);
}
...
function onCheckCustomerType(e, treeId, treeNode) {
	...
	if (name.length > 0 ) name = name.substring(0, name.length-1);
	if (id.length > 0 ) id = id.substring(0, id.length-1);
	$("#mdOrderStatForm").find("#customerTypeName").val(name);
	$("#mdOrderStatForm").find("#customerTypeId").val(id);
}
...
function onCheckProductClass(e, treeId, treeNode) {
	...
	if (name.length > 0 ) name = name.substring(0, name.length-1);
	if (id.length > 0 ) id = id.substring(0, id.length-1);
	$("#mdOrderStatForm").find("#productClassName").val(name);
	$("#mdOrderStatForm").find("#productClassId").val(id);
}
...
function onCheckSubchannel(e, treeId, treeNode) {
	...
	if (name.length > 0 ) name = name.substring(0, name.length-1);
	if (id.length > 0 ) id = id.substring(0, id.length-1);
	$("#mdOrderStatForm").find("#subchannelName").val(name);
	$("#mdOrderStatForm").find("#subchannelId").val(id);
}
...
function onCheckOrderType(e, treeId, treeNode) {
	...
	if (name.length > 0 ) name = name.substring(0, name.length-1);
	if (id.length > 0 ) id = id.substring(0, id.length-1);
	$("#mdOrderStatForm").find("#orderTypeName").val(name);
	$("#mdOrderStatForm").find("#orderTypeId").val(id);
}
...
function onCheckOrderStatus(e, treeId, treeNode) {
	...
	if (name.length > 0 ) name = name.substring(0, name.length-1);
	if (id.length > 0 ) id = id.substring(0, id.length-1);
	$("#mdOrderStatForm").find("#orderStatusName").val(name);
	$("#mdOrderStatForm").find("#orderStatusId").val(id);
}
...
function ajaxFileUpload()
{
	...
	(
		{
			url:'assessStat!upload.action?paramMap.prefix=mdorderstat_product_',//用于文件上传的服务器端请求地址
...
function ajaxFileUploadUserId()
{
	...
	(
		{
			url:'assessStat!uploadUserId.action?paramMap.prefix=mdorderstat_userid_',//用于文件上传的服务器端请求地址
			secureuri:false,//一般设置为false
 ...
 * MD工号excel上传
 */
function ajaxFileUploadMDId()
{
	$("#MDidLoading").show();
	$.ajaxFileUpload
	(
		{
			url:'assessStat!uploadMDId.action?paramMap.prefix=mdorderstat_mdid_',//用于文件上传的服务器端请求地址
			secureuri:false,//一般设置为false
			fileElementId:'fileMDid',//文件上传空间的id属性  <input type="file" id="file" name="file" />
			dataType: 'json',//返回值类型 一般设置为json
			success: function (data, status)  //服务器成功响应处理函数
			{
				if (data==undefined||data["errCode"]==undefined||data["errCode"]!=0) {
				...
				var names = data["value"].split(",");
				$("#localFileMDName").empty();
				$("#localFileMDName").append(names[0]);
				$("#fileMDName").val(names[1]);
				$("#MDidLoading").hide();
			},
			error: function (data, status, e)//服务器响应失败处理函数
			{

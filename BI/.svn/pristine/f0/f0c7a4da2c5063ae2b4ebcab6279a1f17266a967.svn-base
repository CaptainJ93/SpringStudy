$(function(){
	$.getCaches("../cache!getCache.action","SEX,EXCLUDERETURNFLAG",function(cacheKeys){
		var options={
				...
				defaultSearch:false,
				colModel:[
				    {label:"日期",name:"prmdate",width:"100",fixed:true},
				    {label:"星期",name:"weekday",width:"100",fixed:true},
				    ...
				    {label:"PD1",name:"pdname1",width:"100",fixed:true},
				    {label:"PD2",name:"pdname2",width:"100",fixed:true},
				    {label:"PD3",name:"pdname3",width:"100",fixed:true},
				    {label:"SH1",name:"shname1",width:"100",fixed:true},
				    {label:"SH2",name:"shname2",width:"100",fixed:true},
				    {label:"SH3",name:"shname3",width:"100",fixed:true},
				    ...
				    {label:"价格",name:"price",width:"100",fixed:true},
				    ...
				    {label:"单位毛利",name:"maolipmin",width:"100",fixed:true}
				],
				multiselect:false,
				loadonce: false,
				caption:"查询列表",
				searchform: "#pgMorderStatForm",
				sortname:"prmdate desc,shiduan",
				sortorder:"asc",
		...
		$("#pgMorderStatTable").hlGrid(options);
		$("#pgMorderStatTable").closest(".ui-jqgrid-bdiv").css({ 'overflow-x' : 'hidden' });
	...
	$("#refreshBtn").click(function() {
		if($("#prmStartTime").datebox("getValue") == "" || $("#prmStartTime").datebox("getValue") == null || $("#prmEndTime").datebox("getValue") == "" || $("#prmEndTime").datebox("getValue") == null){
			$.messager.alert("操作提示", "请输入节目起始时间!");
			return false;
		}
		
		if(compareDate($("#prmStartTime").datebox("getValue"),$("#prmEndTime").datebox("getValue"))){
			$.messager.alert("操作提示", "节目结束时间应大于开始时间!");
			return;
		}
...
//		}
		...
		if($("#pgMorderStatForm").find("#ageStart").val() != "" && ($("#pgMorderStatForm").find("#ageEnd").val() == null || $("#pgMorderStatForm").find("#ageEnd").val() == "")){
			$.messager.alert("操作提示", "起始年龄和终止年龄必须同时填写!");
			$("#pgMorderStatForm").find("#ageEnd").focus();
			return false;
	 	}
		
		if($("#pgMorderStatForm").find("#ageEnd").val() != "" && ($("#pgMorderStatForm").find("#ageStart").val() == null || $("#pgMorderStatForm").find("#ageStart").val() == "")){
			$.messager.alert("操作提示", "起始年龄和终止年龄必须同时填写!");
			$("#pgMorderStatForm").find("#ageStart").focus();
			return false;
	 	}
		...
		if($("#registerEndTime").datebox("getValue") != "" &&($("#registerStartTime").datebox("getValue") == null || $("#registerStartTime").datebox("getValue") == "")){
			...
			$("#pgMorderStatForm").find("#registerStartTime").focus();
			return false;
		}
		...
		if($("#actualEndTime").datebox("getValue") != "" &&($("#actualStartTime").datebox("getValue") == null || $("#actualStartTime").datebox("getValue") == "")){
			...
			$("#pgMorderStatForm").find("#actualStartTime").focus();
			return false;
		}
		...
		if($("#orderEndTime").datebox("getValue") != "" &&($("#orderStartTime").datebox("getValue") == null || $("#orderStartTime").datebox("getValue") == "")){
			...
			$("#pgMorderStatForm").find("#orderStartTime").focus();
			return false;
		}
		...
		jQuery("#pgMorderStatTable").jqGrid("setGridParam", {url:"channelStat!pgMorderStatList.action"}); 
		$("#pgMorderStatTable").hlGridRefresh();
		$("#pgMorderStatTable").closest(".ui-jqgrid-bdiv").css({ 'overflow-x' : 'scroll' });
	});
	
	$("#resetBtn").click(function(){
		$("#pgMorderStatForm").clearForm();
		//会员等级清空
		if($.fn.zTree.getZTreeObj("levelTree") != null){
			$.fn.zTree.getZTreeObj("levelTree").checkAllNodes(false);
		}
		//性别初始化
		$("#pgMorderStatForm").find("#sex").val(null);
		$("#pgMorderStatForm").find("#excludereturnflag").val("N");
		//会员类型
	...
	$("#clearFileCreatebyid").click(function() {
		...
		$("#clearLevel").click(function() {
			$("#pgMorderStatForm").find("#membershiplevelname").val("");
			$("#pgMorderStatForm").find("#membershiplevelid").val("");
		...
		$("#clearCustomerType").click(function() {
			$("#pgMorderStatForm").find("#customerTypeName").val("");
			$("#pgMorderStatForm").find("#customerTypeId").val("");
		...
		$("#clearProductClass").click(function() {
			$("#pgMorderStatForm").find("#productClassName").val("");
			$("#pgMorderStatForm").find("#productClassId").val("");
		...
		$("#clearSubchannel").click(function() {
			$("#pgMorderStatForm").find("#subchannelName").val("");
			$("#pgMorderStatForm").find("#subchannelId").val("");
		...
		$("#clearOrderType").click(function() {
			$("#pgMorderStatForm").find("#orderTypeName").val("");
			$("#pgMorderStatForm").find("#orderTypeId").val("");
		...
		$("#clearOrderStatus").click(function() {
			$("#pgMorderStatForm").find("#orderStatusName").val("");
			$("#pgMorderStatForm").find("#orderStatusId").val("");
...
function onCheckLvl(e, treeId, treeNode) {
	...
	$("#pgMorderStatForm").find("#membershiplevelname").val(name);
	$("#pgMorderStatForm").find("#membershiplevelid").val(id);
}
...
function onCheckCustomerType(e, treeId, treeNode) {
	...
	if (name.length > 0 ) name = name.substring(0, name.length-1);
	if (id.length > 0 ) id = id.substring(0, id.length-1);
	$("#pgMorderStatForm").find("#customerTypeName").val(name);
	$("#pgMorderStatForm").find("#customerTypeId").val(id);
}
...
function onCheckProductClass(e, treeId, treeNode) {
	...
	if (name.length > 0 ) name = name.substring(0, name.length-1);
	if (id.length > 0 ) id = id.substring(0, id.length-1);
	$("#pgMorderStatForm").find("#productClassName").val(name);
	$("#pgMorderStatForm").find("#productClassId").val(id);
}
...
function onCheckSubchannel(e, treeId, treeNode) {
	...
	if (name.length > 0 ) name = name.substring(0, name.length-1);
	if (id.length > 0 ) id = id.substring(0, id.length-1);
	$("#pgMorderStatForm").find("#subchannelName").val(name);
	$("#pgMorderStatForm").find("#subchannelId").val(id);
}
...
function onCheckOrderType(e, treeId, treeNode) {
	...
	if (name.length > 0 ) name = name.substring(0, name.length-1);
	if (id.length > 0 ) id = id.substring(0, id.length-1);
	$("#pgMorderStatForm").find("#orderTypeName").val(name);
	$("#pgMorderStatForm").find("#orderTypeId").val(id);
}
...
function onCheckOrderStatus(e, treeId, treeNode) {
	...
	if (name.length > 0 ) name = name.substring(0, name.length-1);
	if (id.length > 0 ) id = id.substring(0, id.length-1);
	$("#pgMorderStatForm").find("#orderStatusName").val(name);
	$("#pgMorderStatForm").find("#orderStatusId").val(id);
}
...
function ajaxFileUpload()
{
	...
	(
		{
			url:'channelStat!upload.action?paramMap.prefix=pgmorderstat_product_',//用于文件上传的服务器端请求地址
...
function ajaxFileUploadUserId()
{
	...
	(
		{
			url:'channelStat!uploadUserId.action?paramMap.prefix=pgmorderstat_userid_',//用于文件上传的服务器端请求地址
			secureuri:false,//一般设置为false

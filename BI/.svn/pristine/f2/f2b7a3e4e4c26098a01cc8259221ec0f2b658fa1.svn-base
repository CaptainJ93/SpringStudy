$(function(){
	$.getCaches("../cache!getCache.action","SEX,EXCLUDERETURNFLAG",function(cacheKeys){
		var options={
				...
				loadonce: false,
				caption:"查询列表",
				searchform: "#classOrderStatForm",
				sortname:"fenleiname",
				allowOper : {add:false,del:false,update:false,exportexcel:true}
			};
		
		$("#classOrderStatTable").hlGrid(options);
...
//		}
		...
		if($("#classOrderStatForm").find("#ageStart").val() != "" && ($("#classOrderStatForm").find("#ageEnd").val() == null || $("#classOrderStatForm").find("#ageEnd").val() == "")){
			$.messager.alert("操作提示", "起始年龄和终止年龄必须同时填写!");
			$("#classOrderStatForm").find("#ageEnd").focus();
			return false;
	 	}
		
		if($("#classOrderStatForm").find("#ageEnd").val() != "" && ($("#classOrderStatForm").find("#ageStart").val() == null || $("#classOrderStatForm").find("#ageStart").val() == "")){
			$.messager.alert("操作提示", "起始年龄和终止年龄必须同时填写!");
			$("#classOrderStatForm").find("#ageStart").focus();
			return false;
	 	}
		...
		if($("#registerEndTime").datebox("getValue") != "" &&($("#registerStartTime").datebox("getValue") == null || $("#registerStartTime").datebox("getValue") == "")){
			...
			$("#classOrderStatForm").find("#registerStartTime").focus();
			return false;
		}
		...
		if($("#actualEndTime").datebox("getValue") != "" &&($("#actualStartTime").datebox("getValue") == null || $("#actualStartTime").datebox("getValue") == "")){
			...
			$("#classOrderStatForm").find("#actualStartTime").focus();
			return false;
		}
		...
		if($("#orderEndTime").datebox("getValue") != "" &&($("#orderStartTime").datebox("getValue") == null || $("#orderStartTime").datebox("getValue") == "")){
			...
			$("#classOrderStatForm").find("#orderStartTime").focus();
			return false;
		}
		...
		jQuery("#classOrderStatTable").jqGrid("setGridParam", {url:"productStat!classOrderStatList.action"}); 
		$("#classOrderStatTable").hlGridRefresh();
	});
	
	$("#resetBtn").click(function(){
		$("#classOrderStatForm").clearForm();
		//会员等级清空
		if($.fn.zTree.getZTreeObj("levelTree") != null){
			$.fn.zTree.getZTreeObj("levelTree").checkAllNodes(false);
		}
		//性别初始化
		$("#classOrderStatForm").find("#sex").val(null);
		$("#classOrderStatForm").find("#excludereturnflag").val("N");
		//会员类型
	...
	$("#clearFileCreatebyid").click(function() {
		...
		$("#clearLevel").click(function() {
			$("#classOrderStatForm").find("#membershiplevelname").val("");
			$("#classOrderStatForm").find("#membershiplevelid").val("");
		...
		$("#clearCustomerType").click(function() {
			$("#classOrderStatForm").find("#customerTypeName").val("");
			$("#classOrderStatForm").find("#customerTypeId").val("");
		...
		$("#clearProductClass").click(function() {
			$("#classOrderStatForm").find("#productClassName").val("");
			$("#classOrderStatForm").find("#productClassId").val("");
		...
		$("#clearSubchannel").click(function() {
			$("#classOrderStatForm").find("#subchannelName").val("");
			$("#classOrderStatForm").find("#subchannelId").val("");
		...
		$("#clearOrderType").click(function() {
			$("#classOrderStatForm").find("#orderTypeName").val("");
			$("#classOrderStatForm").find("#orderTypeId").val("");
		...
		$("#clearOrderStatus").click(function() {
			$("#classOrderStatForm").find("#orderStatusName").val("");
			$("#classOrderStatForm").find("#orderStatusId").val("");
...
function onCheckLvl(e, treeId, treeNode) {
	...
	$("#classOrderStatForm").find("#membershiplevelname").val(name);
	$("#classOrderStatForm").find("#membershiplevelid").val(id);
}
...
function onCheckCustomerType(e, treeId, treeNode) {
	...
	if (name.length > 0 ) name = name.substring(0, name.length-1);
	if (id.length > 0 ) id = id.substring(0, id.length-1);
	$("#classOrderStatForm").find("#customerTypeName").val(name);
	$("#classOrderStatForm").find("#customerTypeId").val(id);
}
...
function onCheckProductClass(e, treeId, treeNode) {
	...
	if (name.length > 0 ) name = name.substring(0, name.length-1);
	if (id.length > 0 ) id = id.substring(0, id.length-1);
	$("#classOrderStatForm").find("#productClassName").val(name);
	$("#classOrderStatForm").find("#productClassId").val(id);
}
...
function onCheckSubchannel(e, treeId, treeNode) {
	...
	if (name.length > 0 ) name = name.substring(0, name.length-1);
	if (id.length > 0 ) id = id.substring(0, id.length-1);
	$("#classOrderStatForm").find("#subchannelName").val(name);
	$("#classOrderStatForm").find("#subchannelId").val(id);
}
...
function onCheckOrderType(e, treeId, treeNode) {
	...
	if (name.length > 0 ) name = name.substring(0, name.length-1);
	if (id.length > 0 ) id = id.substring(0, id.length-1);
	$("#classOrderStatForm").find("#orderTypeName").val(name);
	$("#classOrderStatForm").find("#orderTypeId").val(id);
}
...
function onCheckOrderStatus(e, treeId, treeNode) {
	...
	if (name.length > 0 ) name = name.substring(0, name.length-1);
	if (id.length > 0 ) id = id.substring(0, id.length-1);
	$("#classOrderStatForm").find("#orderStatusName").val(name);
	$("#classOrderStatForm").find("#orderStatusId").val(id);
}
...
function ajaxFileUpload()
{
	...
	(
		{
			url:'productStat!upload.action?paramMap.prefix=classorderstat_product_',//用于文件上传的服务器端请求地址
...
function ajaxFileUploadUserId()
{
	...
	(
		{
			url:'productStat!uploadUserId.action?paramMap.prefix=classorderstat_userid_',//用于文件上传的服务器端请求地址
			secureuri:false,//一般设置为false

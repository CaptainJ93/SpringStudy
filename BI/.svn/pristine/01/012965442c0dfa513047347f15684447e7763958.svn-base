$(function(){
	$.getCaches("../cache!getCache.action","SEX,EXCLUDERETURNFLAG",function(cacheKeys){
		var options={
				...
				defaultSearch:false,
				exporturl:"detailStat!exportOrderDetailAllOut.action",//用于数据导出url
				colModel:[
				    ...
				    {label:"项次",name:"itemid"},
				    {label:"商品号",name:"productid"},
				    {label:"商品名",name:"productname"},
				    {label:"数量",name:"quantity"},
				    {label:"售价",name:"price"},
				    {label:"折扣金额",name:"discountamount"},
				    {label:"实际支付金额",name:"productamount"},
				    {label:"订单日期",name:"ordereddate"}
				],
				multiselect:false,
				loadonce: false,
				caption:"查询列表",
				sortname:"ordereddate",
				sortorder:"desc",
				searchform: "#orderDetailAllOutForm",
				allowOper : {add:false,del:false,update:false,exportexcel:true}
			};
		
			$("#orderDetailAllOutTable").hlGrid(options);
...
//		}
		...
		if($("#orderDetailAllOutForm").find("#ageStart").val() != "" && ($("#orderDetailAllOutForm").find("#ageEnd").val() == null || $("#orderDetailAllOutForm").find("#ageEnd").val() == "")){
			$.messager.alert("操作提示", "起始年龄和终止年龄必须同时填写!");
			$("#orderDetailAllOutForm").find("#ageEnd").focus();
			return false;
	 	}
		
		if($("#orderDetailAllOutForm").find("#ageEnd").val() != "" && ($("#orderDetailAllOutForm").find("#ageStart").val() == null || $("#orderDetailAllOutForm").find("#ageStart").val() == "")){
			$.messager.alert("操作提示", "起始年龄和终止年龄必须同时填写!");
			$("#orderDetailAllOutForm").find("#ageStart").focus();
			return false;
	 	}
		...
		if($("#registerEndTime").datebox("getValue") != "" &&($("#registerStartTime").datebox("getValue") == null || $("#registerStartTime").datebox("getValue") == "")){
			...
			$("#orderDetailAllOutForm").find("#registerStartTime").focus();
			return false;
		}
		...
		if($("#actualEndTime").datebox("getValue") != "" &&($("#actualStartTime").datebox("getValue") == null || $("#actualStartTime").datebox("getValue") == "")){
			...
			$("#orderDetailAllOutForm").find("#actualStartTime").focus();
			return false;
		}
		...
		if($("#orderEndTime").datebox("getValue") != "" &&($("#orderStartTime").datebox("getValue") == null || $("#orderStartTime").datebox("getValue") == "")){
			...
			$("#orderDetailAllOutForm").find("#orderStartTime").focus();
			return false;
		}
		...
		jQuery("#orderDetailAllOutTable").jqGrid("setGridParam", {url:"detailStat!orderDetailAllOutList.action"}); 
		$("#orderDetailAllOutTable").hlGridRefresh();
	});
	
	$("#resetBtn").click(function(){
		$("#orderDetailAllOutForm").clearForm();
		//会员等级清空
		if($.fn.zTree.getZTreeObj("levelTree") != null){
			$.fn.zTree.getZTreeObj("levelTree").checkAllNodes(false);
		}
		//性别初始化
		$("#orderDetailAllOutForm").find("#sex").val(null);
		$("#orderDetailAllOutForm").find("#excludereturnflag").val("N");
		//会员类型
	...
	$("#showPhone").change(function() {
		...
		if (!$("#showPhone").attr("checked")) {
			jQuery("#orderDetailAllOutTable").setGridParam({"postData":{"paramMap.sensitiveData":$("#sensitiveData").val()}}).hideCol("mobile").trigger("reloadGrid");
        }else if($("#showPhone").attr("checked")){
        	jQuery("#orderDetailAllOutTable").setGridParam({"postData":{"paramMap.sensitiveData":$("#sensitiveData").val()}}).showCol("mobile").trigger("reloadGrid");
        }
		$("#orderDetailAllOutTable").hlGridAdjustWidth();
    });
	
	$("#showIdCard").change(function() {
		setSensitiveDataFlag();
		if (!$("#showIdCard").attr("checked")) {
			jQuery("#orderDetailAllOutTable").setGridParam({"postData":{"paramMap.sensitiveData":$("#sensitiveData").val()}}).hideCol("cardid").trigger("reloadGrid");
        }else if($("#showIdCard").attr("checked")){
        	jQuery("#orderDetailAllOutTable").setGridParam({"postData":{"paramMap.sensitiveData":$("#sensitiveData").val()}}).showCol("cardid").trigger("reloadGrid");
        }
		$("#orderDetailAllOutTable").hlGridAdjustWidth();
    });
	...
	$("#showAddress").change(function() {
		...
		if (!$("#showAddress").attr("checked")) {
			jQuery("#orderDetailAllOutTable").setGridParam({"postData":{"paramMap.sensitiveData":$("#sensitiveData").val()}}).hideCol("address2").trigger("reloadGrid");
        }else if($("#showAddress").attr("checked")){
        	jQuery("#orderDetailAllOutTable").setGridParam({"postData":{"paramMap.sensitiveData":$("#sensitiveData").val()}}).showCol("address2").trigger("reloadGrid");
        }
		$("#orderDetailAllOutTable").hlGridAdjustWidth();
    });
	...
	$("#clearFileCreatebyid").click(function() {
		...
		$("#clearLevel").click(function() {
			$("#orderDetailAllOutForm").find("#membershiplevelname").val("");
			$("#orderDetailAllOutForm").find("#membershiplevelid").val("");
		...
		$("#clearCustomerType").click(function() {
			$("#orderDetailAllOutForm").find("#customerTypeName").val("");
			$("#orderDetailAllOutForm").find("#customerTypeId").val("");
		...
		$("#clearProductClass").click(function() {
			$("#orderDetailAllOutForm").find("#productClassName").val("");
			$("#orderDetailAllOutForm").find("#productClassId").val("");
		...
		$("#clearSubchannel").click(function() {
			$("#orderDetailAllOutForm").find("#subchannelName").val("");
			$("#orderDetailAllOutForm").find("#subchannelId").val("");
		...
		$("#clearOrderType").click(function() {
			$("#orderDetailAllOutForm").find("#orderTypeName").val("");
			$("#orderDetailAllOutForm").find("#orderTypeId").val("");
		...
		$("#clearOrderStatus").click(function() {
			$("#orderDetailAllOutForm").find("#orderStatusName").val("");
			$("#orderDetailAllOutForm").find("#orderStatusId").val("");
...
function onCheckLvl(e, treeId, treeNode) {
	...
	$("#orderDetailAllOutForm").find("#membershiplevelname").val(name);
	$("#orderDetailAllOutForm").find("#membershiplevelid").val(id);
}
...
function onCheckCustomerType(e, treeId, treeNode) {
	...
	if (name.length > 0 ) name = name.substring(0, name.length-1);
	if (id.length > 0 ) id = id.substring(0, id.length-1);
	$("#orderDetailAllOutForm").find("#customerTypeName").val(name);
	$("#orderDetailAllOutForm").find("#customerTypeId").val(id);
}
...
function onCheckProductClass(e, treeId, treeNode) {
	...
	if (name.length > 0 ) name = name.substring(0, name.length-1);
	if (id.length > 0 ) id = id.substring(0, id.length-1);
	$("#orderDetailAllOutForm").find("#productClassName").val(name);
	$("#orderDetailAllOutForm").find("#productClassId").val(id);
}
...
function onCheckSubchannel(e, treeId, treeNode) {
	...
	if (name.length > 0 ) name = name.substring(0, name.length-1);
	if (id.length > 0 ) id = id.substring(0, id.length-1);
	$("#orderDetailAllOutForm").find("#subchannelName").val(name);
	$("#orderDetailAllOutForm").find("#subchannelId").val(id);
}
...
function onCheckOrderType(e, treeId, treeNode) {
	...
	if (name.length > 0 ) name = name.substring(0, name.length-1);
	if (id.length > 0 ) id = id.substring(0, id.length-1);
	$("#orderDetailAllOutForm").find("#orderTypeName").val(name);
	$("#orderDetailAllOutForm").find("#orderTypeId").val(id);
}
...
function onCheckOrderStatus(e, treeId, treeNode) {
	...
	if (name.length > 0 ) name = name.substring(0, name.length-1);
	if (id.length > 0 ) id = id.substring(0, id.length-1);
	$("#orderDetailAllOutForm").find("#orderStatusName").val(name);
	$("#orderDetailAllOutForm").find("#orderStatusId").val(id);
}
...
function ajaxFileUpload()
{
	...
	(
		{
			url:'detailStat!upload.action?paramMap.prefix=orderdetailallout_product_',//用于文件上传的服务器端请求地址
...
function ajaxFileUploadUserId()
{
	...
	(
		{
			url:'detailStat!uploadUserId.action?paramMap.prefix=orderdetailallout_userid_',//用于文件上传的服务器端请求地址
			secureuri:false,//一般设置为false

...
<script src="js/customerInfoAllOutList.js" type="text/javascript"></script>
<style>
...
<body style="overflow-x: hidden;">
	<div class="mainSilde mainBlock">
		...
		<form action="" id="customerInfoAllOutForm" method="post">
		<%
			String menuName = "";
		...
		<input type="hidden" name="sensitiveData" id="sensitiveData"/><!-- 用于判断敏感数据导出 -->
			   ...
			   <s:if test='#session.user.getSensitiveData().get("PHONE")=="true" || #session.user.getSensitiveData().get("IDCARD")=="true" || #session.user.getSensitiveData().get("ADDRESS")=="true"'>
			   		<li>
			   			...
			   			<s:if test='#session.user.getSensitiveData().get("ADDRESS")=="true"'>
			   				<input type="checkbox" name="详细地址显示" id="showAddress"/>详细地址
			   			</s:if>
			   		</li>
			   </s:if>	
			</ul>
		...
		<table id="customerInfoAllOutTable"></table>
	</div>
</body>
</html>
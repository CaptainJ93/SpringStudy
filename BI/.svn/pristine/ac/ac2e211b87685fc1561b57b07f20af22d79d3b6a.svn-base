$(function(){
	...
	var options={
			...
			sortorder:"desc",
			searchform: "#complaintResStatForm",
			allowOper : {add:false,del:false,update:false,exportexcel:true}
		};
	$("#complaintResStatTable").hlGrid(options);
...
//		}
		...
		if($("#orderEndTime").datebox("getValue") != "" &&($("#orderStartTime").datebox("getValue") == null || $("#orderStartTime").datebox("getValue") == "")){
			...
			$("#complaintResStatForm").find("#orderStartTime").focus();
			return false;
		}
		...
		jQuery("#complaintResStatTable").jqGrid("setGridParam", {url:"customerStat!complaintResStatList.action"}); 
		$("#complaintResStatTable").hlGridRefresh();
	});
	
	$("#resetBtn").click(function(){
		$("#complaintResStatForm").clearForm();
		//会员等级清空
	...
	$("#clearFileHandlebyid").click(function() {
		...
		$("#clearLevel").click(function() {
			$("#complaintResStatForm").find("#membershiplevelname").val("");
			$("#complaintResStatForm").find("#membershiplevelid").val("");
		...
		$("#clearCustomerType").click(function() {
			$("#complaintResStatForm").find("#customerTypeName").val("");
			$("#complaintResStatForm").find("#customerTypeId").val("");
		...
		$("#clearReturnReason").click(function() {
			$("#complaintResStatForm").find("#returnReasonName").val("");
			$("#complaintResStatForm").find("#returnReasonId").val("");
		...
		$("#clearComplaintCategory").click(function() {
			$("#complaintResStatForm").find("#complaintCategoryName").val("");
			$("#complaintResStatForm").find("#complaintCategoryId").val("");
...
function onCheckLvl(e, treeId, treeNode) {
	...
	$("#complaintResStatForm").find("#membershiplevelname").val(name);
	$("#complaintResStatForm").find("#membershiplevelid").val(id);
}
...
function onCheckCustomerType(e, treeId, treeNode) {
	...
	if (name.length > 0 ) name = name.substring(0, name.length-1);
	if (id.length > 0 ) id = id.substring(0, id.length-1);
	$("#complaintResStatForm").find("#customerTypeName").val(name);
	$("#complaintResStatForm").find("#customerTypeId").val(id);
}
...
function onCheckReturnReason(e, treeId, treeNode) {
	...
	if (name.length > 0 ) name = name.substring(0, name.length-1);
	if (id.length > 0 ) id = id.substring(0, id.length-1);
	$("#complaintResStatForm").find("#returnReasonName").val(name);
	$("#complaintResStatForm").find("#returnReasonId").val(id);
}
...
function onCheckComplaintCategory(e, treeId, treeNode) {
	...
	if (name.length > 0 ) name = name.substring(0, name.length-1);
	if (id.length > 0 ) id = id.substring(0, id.length-1);
	$("#complaintResStatForm").find("#complaintCategoryName").val(name);
	$("#complaintResStatForm").find("#complaintCategoryId").val(id);
}
...
function ajaxFileUpload()
{
	...
	(
		{
			url:'customerStat!upload.action?paramMap.prefix=complaintResStat_product_',//用于文件上传的服务器端请求地址
...
function ajaxFileUploadUserId()
{
	...
	(
		{
			url:'customerStat!uploadUserId.action?paramMap.prefix=complaintResStat_userid_',//用于文件上传的服务器端请求地址
			secureuri:false,//一般设置为false
...
function ajaxFileUploadUserId2()
{
	...
	(
		{
			url:'customerStat!uploadHandleUserId.action?paramMap.prefix=complaintResStat_userid2_',//用于文件上传的服务器端请求地址
			secureuri:false,//一般设置为false

	...
	$.getCaches("../cache!getCache.action","SEX,EXCLUDERETURNFLAG",function(cacheKeys){
		var options={
				...
				colModel:[
				    ...
				    {label:"购买过的会员数",name:"purchasedcustcnt"},
				    {label:"占比",name:"pct_total"}
				],
				multiselect:false,
				loadonce: false,
				caption:"查询列表",
				sortname:"purchasedcustcnt",
				sortorder:"desc",
				searchform: "#productCustStatForm",
				allowOper : {add:false,del:false,update:false,exportexcel:true}
			};
		$("#productCustStatTable").hlGrid(options);
...
//		}
		...
		if($("#productCustStatForm").find("#ageStart").val() != "" && ($("#productCustStatForm").find("#ageEnd").val() == null || $("#productCustStatForm").find("#ageEnd").val() == "")){
			$.messager.alert("操作提示", "起始年龄和终止年龄必须同时填写!");
			$("#productCustStatForm").find("#ageEnd").focus();
			return false;
	 	}
		
		if($("#productCustStatForm").find("#ageEnd").val() != "" && ($("#productCustStatForm").find("#ageStart").val() == null || $("#productCustStatForm").find("#ageStart").val() == "")){
			$.messager.alert("操作提示", "起始年龄和终止年龄必须同时填写!");
			$("#productCustStatForm").find("#ageStart").focus();
			return false;
	 	}
		...
		if($("#registerEndTime").datebox("getValue") != "" &&($("#registerStartTime").datebox("getValue") == null || $("#registerStartTime").datebox("getValue") == "")){
			...
			$("#productCustStatForm").find("#registerStartTime").focus();
			return false;
		}
		...
		if($("#actualEndTime").datebox("getValue") != "" &&($("#actualStartTime").datebox("getValue") == null || $("#actualStartTime").datebox("getValue") == "")){
			...
			$("#productCustStatForm").find("#actualStartTime").focus();
			return false;
		}
		...
		if($("#orderEndTime").datebox("getValue") != "" &&($("#orderStartTime").datebox("getValue") == null || $("#orderStartTime").datebox("getValue") == "")){
			...
			$("#productCustStatForm").find("#orderStartTime").focus();
			return false;
		}
		...
		jQuery("#productCustStatTable").jqGrid("setGridParam", {url:"customerStat!productCustStatList.action"}); 
		$("#productCustStatTable").hlGridRefresh();
	});
	
	$("#resetBtn").click(function(){
		$("#productCustStatForm").clearForm();
		//会员等级清空
		if($.fn.zTree.getZTreeObj("levelTree") != null){
			$.fn.zTree.getZTreeObj("levelTree").checkAllNodes(false);
		}
		//性别初始化
		$("#productCustStatForm").find("#sex").val(null);
		$("#productCustStatForm").find("#excludereturnflag").val("N");
		//会员类型
	...
	$("#clearFileCreatebyid").click(function() {
		...
		$("#clearLevel").click(function() {
			$("#productCustStatForm").find("#membershiplevelname").val("");
			$("#productCustStatForm").find("#membershiplevelid").val("");
		...
		$("#clearCustomerType").click(function() {
			$("#productCustStatForm").find("#customerTypeName").val("");
			$("#productCustStatForm").find("#customerTypeId").val("");
		...
		$("#clearProductClass").click(function() {
			$("#productCustStatForm").find("#productClassName").val("");
			$("#productCustStatForm").find("#productClassId").val("");
		...
		$("#clearSubchannel").click(function() {
			$("#productCustStatForm").find("#subchannelName").val("");
			$("#productCustStatForm").find("#subchannelId").val("");
		...
		$("#clearOrderType").click(function() {
			$("#productCustStatForm").find("#orderTypeName").val("");
			$("#productCustStatForm").find("#orderTypeId").val("");
		...
		$("#clearOrderStatus").click(function() {
			$("#productCustStatForm").find("#orderStatusName").val("");
			$("#productCustStatForm").find("#orderStatusId").val("");
...
function onCheckLvl(e, treeId, treeNode) {
	...
	$("#productCustStatForm").find("#membershiplevelname").val(name);
	$("#productCustStatForm").find("#membershiplevelid").val(id);
}
...
function onCheckCustomerType(e, treeId, treeNode) {
	...
	if (name.length > 0 ) name = name.substring(0, name.length-1);
	if (id.length > 0 ) id = id.substring(0, id.length-1);
	$("#productCustStatForm").find("#customerTypeName").val(name);
	$("#productCustStatForm").find("#customerTypeId").val(id);
}
...
function onCheckProductClass(e, treeId, treeNode) {
	...
	if (name.length > 0 ) name = name.substring(0, name.length-1);
	if (id.length > 0 ) id = id.substring(0, id.length-1);
	$("#productCustStatForm").find("#productClassName").val(name);
	$("#productCustStatForm").find("#productClassId").val(id);
}
...
function onCheckSubchannel(e, treeId, treeNode) {
	...
	if (name.length > 0 ) name = name.substring(0, name.length-1);
	if (id.length > 0 ) id = id.substring(0, id.length-1);
	$("#productCustStatForm").find("#subchannelName").val(name);
	$("#productCustStatForm").find("#subchannelId").val(id);
}
...
function onCheckOrderType(e, treeId, treeNode) {
	...
	if (name.length > 0 ) name = name.substring(0, name.length-1);
	if (id.length > 0 ) id = id.substring(0, id.length-1);
	$("#productCustStatForm").find("#orderTypeName").val(name);
	$("#productCustStatForm").find("#orderTypeId").val(id);
}
...
function onCheckOrderStatus(e, treeId, treeNode) {
	...
	if (name.length > 0 ) name = name.substring(0, name.length-1);
	if (id.length > 0 ) id = id.substring(0, id.length-1);
	$("#productCustStatForm").find("#orderStatusName").val(name);
	$("#productCustStatForm").find("#orderStatusId").val(id);
}
...
function ajaxFileUpload()
{
	...
	(
		{
			url:'customerStat!upload.action?paramMap.prefix=productCustStat_product_',//用于文件上传的服务器端请求地址
...
function ajaxFileUploadUserId()
{
	...
	(
		{
			url:'customerStat!uploadUserId.action?paramMap.prefix=productCustStat_userid_',//用于文件上传的服务器端请求地址
			secureuri:false,//一般设置为false

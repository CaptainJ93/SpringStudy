$(function(){
	$.getCaches("../cache!getCache.action","SEX,EXCLUDERETURNFLAG",function(cacheKeys){
		var options={
				...
				sortorder:"desc",
				searchform: "#areaCustStatForm",
				allowOper : {add:false,del:false,update:false,exportexcel:true}
			};
		$("#areaCustStatTable").hlGrid(options);
...
//		}
		...
		if($("#areaCustStatForm").find("#ageStart").val() != "" && ($("#areaCustStatForm").find("#ageEnd").val() == null || $("#areaCustStatForm").find("#ageEnd").val() == "")){
			$.messager.alert("操作提示", "起始年龄和终止年龄必须同时填写!");
			$("#areaCustStatForm").find("#ageEnd").focus();
			return false;
	 	}
		
		if($("#areaCustStatForm").find("#ageEnd").val() != "" && ($("#areaCustStatForm").find("#ageStart").val() == null || $("#areaCustStatForm").find("#ageStart").val() == "")){
			$.messager.alert("操作提示", "起始年龄和终止年龄必须同时填写!");
			$("#areaCustStatForm").find("#ageStart").focus();
			return false;
	 	}
		...
		if($("#registerEndTime").datebox("getValue") != "" &&($("#registerStartTime").datebox("getValue") == null || $("#registerStartTime").datebox("getValue") == "")){
			...
			$("#areaCustStatForm").find("#registerStartTime").focus();
			return false;
		}
		...
		if($("#actualEndTime").datebox("getValue") != "" &&($("#actualStartTime").datebox("getValue") == null || $("#actualStartTime").datebox("getValue") == "")){
			...
			$("#areaCustStatForm").find("#actualStartTime").focus();
			return false;
		}
		...
		if($("#orderEndTime").datebox("getValue") != "" &&($("#orderStartTime").datebox("getValue") == null || $("#orderStartTime").datebox("getValue") == "")){
			...
			$("#areaCustStatForm").find("#orderStartTime").focus();
			return false;
		}
		...
		jQuery("#areaCustStatTable").jqGrid("setGridParam", {url:"customerStat!areaCustStatList.action"}); 
		$("#areaCustStatTable").hlGridRefresh();
	});
	
	$("#resetBtn").click(function(){
		$("#areaCustStatForm").clearForm();
		//地区清空
		...
		//性别初始化
		$("#areaCustStatForm").find("#sex").val(null);
		$("#areaCustStatForm").find("#excludereturnflag").val("N");
		//会员类型
	...
	$("#clearAreaZipCode").click(function() {
		$("#areaCustStatForm").find("#areaZipCodeName").val("");
		$("#areaCustStatForm").find("#areaZipCodeId").val("");
	 ...
	 */
		...
		$("#clearLevel").click(function() {
			$("#areaCustStatForm").find("#membershiplevelname").val("");
			$("#areaCustStatForm").find("#membershiplevelid").val("");
		...
		$("#clearCustomerType").click(function() {
			$("#areaCustStatForm").find("#customerTypeName").val("");
			$("#areaCustStatForm").find("#customerTypeId").val("");
		...
		$("#clearProductClass").click(function() {
			$("#areaCustStatForm").find("#productClassName").val("");
			$("#areaCustStatForm").find("#productClassId").val("");
		...
		$("#clearSubchannel").click(function() {
			$("#areaCustStatForm").find("#subchannelName").val("");
			$("#areaCustStatForm").find("#subchannelId").val("");
		...
		$("#clearOrderType").click(function() {
			$("#areaCustStatForm").find("#orderTypeName").val("");
			$("#areaCustStatForm").find("#orderTypeId").val("");
		...
		$("#clearOrderStatus").click(function() {
			$("#areaCustStatForm").find("#orderStatusName").val("");
			$("#areaCustStatForm").find("#orderStatusId").val("");
...
function onCheckAreaZipCode(e, treeId, treeNode) {
	...
	$("#areaCustStatForm").find("#areaZipCodeName").val(name);
	$("#areaCustStatForm").find("#areaZipCodeId").val(id);
}
...
function onCheckLvl(e, treeId, treeNode) {
	...
	if (name.length > 0 ) name = name.substring(0, name.length-1);
	if (id.length > 0 ) id = id.substring(0, id.length-1);
	$("#areaCustStatForm").find("#membershiplevelname").val(name);
	$("#areaCustStatForm").find("#membershiplevelid").val(id);
}
...
function onCheckCustomerType(e, treeId, treeNode) {
	...
	if (name.length > 0 ) name = name.substring(0, name.length-1);
	if (id.length > 0 ) id = id.substring(0, id.length-1);
	$("#areaCustStatForm").find("#customerTypeName").val(name);
	$("#areaCustStatForm").find("#customerTypeId").val(id);
}
...
function onCheckProductClass(e, treeId, treeNode) {
	...
	if (name.length > 0 ) name = name.substring(0, name.length-1);
	if (id.length > 0 ) id = id.substring(0, id.length-1);
	$("#areaCustStatForm").find("#productClassName").val(name);
	$("#areaCustStatForm").find("#productClassId").val(id);
}
...
function onCheckSubchannel(e, treeId, treeNode) {
	...
	if (name.length > 0 ) name = name.substring(0, name.length-1);
	if (id.length > 0 ) id = id.substring(0, id.length-1);
	$("#areaCustStatForm").find("#subchannelName").val(name);
	$("#areaCustStatForm").find("#subchannelId").val(id);
}
...
function onCheckOrderType(e, treeId, treeNode) {
	...
	if (name.length > 0 ) name = name.substring(0, name.length-1);
	if (id.length > 0 ) id = id.substring(0, id.length-1);
	$("#areaCustStatForm").find("#orderTypeName").val(name);
	$("#areaCustStatForm").find("#orderTypeId").val(id);
}
...
function onCheckOrderStatus(e, treeId, treeNode) {
	...
	if (name.length > 0 ) name = name.substring(0, name.length-1);
	if (id.length > 0 ) id = id.substring(0, id.length-1);
	$("#areaCustStatForm").find("#orderStatusName").val(name);
	$("#areaCustStatForm").find("#orderStatusId").val(id);
}
...
function ajaxFileUpload()
{
	...
	(
		{
			url:'customerStat!upload.action?paramMap.prefix=areaCustStat_product_',//用于文件上传的服务器端请求地址
...
function ajaxFileUploadUserId()
{
	...
	(
		{
			url:'customerStat!uploadUserId.action?paramMap.prefix=areaCustStat_userid_',//用于文件上传的服务器端请求地址
			secureuri:false,//一般设置为false

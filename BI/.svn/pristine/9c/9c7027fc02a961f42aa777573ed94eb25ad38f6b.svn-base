var tabpanel;
$(function(){
	$.getCaches("../cache!getCache.action","SEX,EXCLUDERETURNFLAG,STAT_CYCLE_TIME",function(cacheKeys){
		var options={
				...
				loadonce: false,
				caption:"查询列表",
				searchform: "#timeOrderStatForm",
				sortname:"orderdate",
				sortorder:"desc",
				allowOper : {add:false,del:false,update:false,exportexcel:true}
			};
		
		$("#timeOrderStatTable").hlGrid(options);
...
//		}
		...
		if($("#timeOrderStatForm").find("#ageStart").val() != "" && ($("#timeOrderStatForm").find("#ageEnd").val() == null || $("#timeOrderStatForm").find("#ageEnd").val() == "")){
			$.messager.alert("操作提示", "起始年龄和终止年龄必须同时填写!");
			$("#timeOrderStatForm").find("#ageEnd").focus();
			return false;
	 	}
		
		if($("#timeOrderStatForm").find("#ageEnd").val() != "" && ($("#timeOrderStatForm").find("#ageStart").val() == null || $("#timeOrderStatForm").find("#ageStart").val() == "")){
			$.messager.alert("操作提示", "起始年龄和终止年龄必须同时填写!");
			$("#timeOrderStatForm").find("#ageStart").focus();
			return false;
	 	}
		...
		if($("#registerEndTime").datebox("getValue") != "" &&($("#registerStartTime").datebox("getValue") == null || $("#registerStartTime").datebox("getValue") == "")){
			...
			$("#timeOrderStatForm").find("#registerStartTime").focus();
			return false;
		}
		...
		if($("#actualEndTime").datebox("getValue") != "" &&($("#actualStartTime").datebox("getValue") == null || $("#actualStartTime").datebox("getValue") == "")){
			...
			$("#timeOrderStatForm").find("#actualStartTime").focus();
			return false;
		}
		...
		if($("#orderEndTime").datebox("getValue") != "" &&($("#orderStartTime").datebox("getValue") == null || $("#orderStartTime").datebox("getValue") == "")){
			...
			$("#timeOrderStatForm").find("#orderStartTime").focus();
			return false;
		}
		...
		jQuery("#timeOrderStatTable").jqGrid("setGridParam", {url:"channelStat!timeOrderStatList.action"}); 
		$("#timeOrderStatTable").hlGridRefresh();
	});
	
	$("#resetBtn").click(function(){
		$("#timeOrderStatForm").clearForm();
		//会员等级清空
		if($.fn.zTree.getZTreeObj("levelTree") != null){
			$.fn.zTree.getZTreeObj("levelTree").checkAllNodes(false);
		}
		//性别初始化
		$("#timeOrderStatForm").find("#sex").val(null);
		$("#timeOrderStatForm").find("#excludereturnflag").val("N");
	...
	$("#clearFileCreatebyid").click(function() {
		...
		$("#clearLevel").click(function() {
			$("#timeOrderStatForm").find("#membershiplevelname").val("");
			$("#timeOrderStatForm").find("#membershiplevelid").val("");
		...
		$("#clearCustomerType").click(function() {
			$("#timeOrderStatForm").find("#customerTypeName").val("");
			$("#timeOrderStatForm").find("#customerTypeId").val("");
		...
		$("#clearProductClass").click(function() {
			$("#timeOrderStatForm").find("#productClassName").val("");
			$("#timeOrderStatForm").find("#productClassId").val("");
		...
		$("#clearSubchannel").click(function() {
			$("#timeOrderStatForm").find("#subchannelName").val("");
			$("#timeOrderStatForm").find("#subchannelId").val("");
		...
		$("#clearOrderType").click(function() {
			$("#timeOrderStatForm").find("#orderTypeName").val("");
			$("#timeOrderStatForm").find("#orderTypeId").val("");
		...
		$("#clearOrderStatus").click(function() {
			$("#timeOrderStatForm").find("#orderStatusName").val("");
			$("#timeOrderStatForm").find("#orderStatusId").val("");
...
function onCheckLvl(e, treeId, treeNode) {
	...
	$("#timeOrderStatForm").find("#membershiplevelname").val(name);
	$("#timeOrderStatForm").find("#membershiplevelid").val(id);
}
...
function onCheckCustomerType(e, treeId, treeNode) {
	...
	if (name.length > 0 ) name = name.substring(0, name.length-1);
	if (id.length > 0 ) id = id.substring(0, id.length-1);
	$("#timeOrderStatForm").find("#customerTypeName").val(name);
	$("#timeOrderStatForm").find("#customerTypeId").val(id);
}
...
function onCheckProductClass(e, treeId, treeNode) {
	...
	if (name.length > 0 ) name = name.substring(0, name.length-1);
	if (id.length > 0 ) id = id.substring(0, id.length-1);
	$("#timeOrderStatForm").find("#productClassName").val(name);
	$("#timeOrderStatForm").find("#productClassId").val(id);
}
...
function onCheckSubchannel(e, treeId, treeNode) {
	...
	if (name.length > 0 ) name = name.substring(0, name.length-1);
	if (id.length > 0 ) id = id.substring(0, id.length-1);
	$("#timeOrderStatForm").find("#subchannelName").val(name);
	$("#timeOrderStatForm").find("#subchannelId").val(id);
}
...
function onCheckOrderType(e, treeId, treeNode) {
	...
	if (name.length > 0 ) name = name.substring(0, name.length-1);
	if (id.length > 0 ) id = id.substring(0, id.length-1);
	$("#timeOrderStatForm").find("#orderTypeName").val(name);
	$("#timeOrderStatForm").find("#orderTypeId").val(id);
}
...
function onCheckOrderStatus(e, treeId, treeNode) {
	...
	if (name.length > 0 ) name = name.substring(0, name.length-1);
	if (id.length > 0 ) id = id.substring(0, id.length-1);
	$("#timeOrderStatForm").find("#orderStatusName").val(name);
	$("#timeOrderStatForm").find("#orderStatusId").val(id);
}
...
function ajaxFileUpload()
{
	...
	(
		{
			url:'channelStat!upload.action?paramMap.prefix=timeOrderStat_product_',//用于文件上传的服务器端请求地址
...
function ajaxFileUploadUserId()
{
	...
	(
		{
			url:'channelStat!uploadUserId.action?paramMap.prefix=timeOrderStat_userid_',//用于文件上传的服务器端请求地址
			secureuri:false,//一般设置为false

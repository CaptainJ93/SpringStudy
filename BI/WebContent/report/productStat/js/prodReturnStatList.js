	...
	$.getCaches("../cache!getCache.action","SEX,EXCLUDERETURNFLAG",function(cacheKeys){
		var options={
				...
				colModel:[
				    ...
				    {label:"销退主因",name:"returnreseason"},
				    {label:"商品件数",name:"productcount"}
				],
				multiselect:false,
				loadonce: false,
				caption:"查询列表",
				searchform: "#prodReturnStatForm",
				sortname:"supperlyid",
				allowOper : {add:false,del:false,update:false,exportexcel:true}
			};
		
		$("#prodReturnStatTable").hlGrid(options);
...
//		}
		...
		if($("#prodReturnStatForm").find("#ageStart").val() != "" && ($("#prodReturnStatForm").find("#ageEnd").val() == null || $("#prodReturnStatForm").find("#ageEnd").val() == "")){
			$.messager.alert("操作提示", "起始年龄和终止年龄必须同时填写!");
			$("#prodReturnStatForm").find("#ageEnd").focus();
			return false;
	 	}
		
		if($("#prodReturnStatForm").find("#ageEnd").val() != "" && ($("#prodReturnStatForm").find("#ageStart").val() == null || $("#prodReturnStatForm").find("#ageStart").val() == "")){
			$.messager.alert("操作提示", "起始年龄和终止年龄必须同时填写!");
			$("#prodReturnStatForm").find("#ageStart").focus();
			return false;
	 	}
		...
		if($("#registerEndTime").datebox("getValue") != "" &&($("#registerStartTime").datebox("getValue") == null || $("#registerStartTime").datebox("getValue") == "")){
			...
			$("#prodReturnStatForm").find("#registerStartTime").focus();
			return false;
		}
		...
		if($("#actualEndTime").datebox("getValue") != "" &&($("#actualStartTime").datebox("getValue") == null || $("#actualStartTime").datebox("getValue") == "")){
			...
			$("#prodReturnStatForm").find("#actualStartTime").focus();
			return false;
		}
		...
		if($("#orderEndTime").datebox("getValue") != "" &&($("#orderStartTime").datebox("getValue") == null || $("#orderStartTime").datebox("getValue") == "")){
			...
			$("#prodReturnStatForm").find("#orderStartTime").focus();
			return false;
		}
		...
		jQuery("#prodReturnStatTable").jqGrid("setGridParam", {url:"productStat!prodReturnStatList.action"}); 
		$("#prodReturnStatTable").hlGridRefresh();
	});
	
	$("#resetBtn").click(function(){
		$("#prodReturnStatForm").clearForm();
		//会员等级清空
		if($.fn.zTree.getZTreeObj("levelTree") != null){
			$.fn.zTree.getZTreeObj("levelTree").checkAllNodes(false);
		}
		//性别初始化
		$("#prodReturnStatForm").find("#sex").val(null);
		$("#prodReturnStatForm").find("#excludereturnflag").val("N");
		//会员类型
	...
	$("#clearFileCreatebyid").click(function() {
		...
		$("#clearLevel").click(function() {
			$("#prodReturnStatForm").find("#membershiplevelname").val("");
			$("#prodReturnStatForm").find("#membershiplevelid").val("");
		...
		$("#clearCustomerType").click(function() {
			$("#prodReturnStatForm").find("#customerTypeName").val("");
			$("#prodReturnStatForm").find("#customerTypeId").val("");
		...
		$("#clearProductClass").click(function() {
			$("#prodReturnStatForm").find("#productClassName").val("");
			$("#prodReturnStatForm").find("#productClassId").val("");
		...
		$("#clearSubchannel").click(function() {
			$("#prodReturnStatForm").find("#subchannelName").val("");
			$("#prodReturnStatForm").find("#subchannelId").val("");
		...
		$("#clearOrderType").click(function() {
			$("#prodReturnStatForm").find("#orderTypeName").val("");
			$("#prodReturnStatForm").find("#orderTypeId").val("");
		...
		$("#clearOrderStatus").click(function() {
			$("#prodReturnStatForm").find("#orderStatusName").val("");
			$("#prodReturnStatForm").find("#orderStatusId").val("");
		...
		$("#clearReturnReason").click(function() {
			$("#prodReturnStatForm").find("#returnReasonName").val("");
			$("#prodReturnStatForm").find("#returnReasonId").val("");
...
function onCheckLvl(e, treeId, treeNode) {
	...
	$("#prodReturnStatForm").find("#membershiplevelname").val(name);
	$("#prodReturnStatForm").find("#membershiplevelid").val(id);
}
...
function onCheckCustomerType(e, treeId, treeNode) {
	...
	if (name.length > 0 ) name = name.substring(0, name.length-1);
	if (id.length > 0 ) id = id.substring(0, id.length-1);
	$("#prodReturnStatForm").find("#customerTypeName").val(name);
	$("#prodReturnStatForm").find("#customerTypeId").val(id);
}
...
function onCheckProductClass(e, treeId, treeNode) {
	...
	if (name.length > 0 ) name = name.substring(0, name.length-1);
	if (id.length > 0 ) id = id.substring(0, id.length-1);
	$("#prodReturnStatForm").find("#productClassName").val(name);
	$("#prodReturnStatForm").find("#productClassId").val(id);
}
...
function onCheckSubchannel(e, treeId, treeNode) {
	...
	if (name.length > 0 ) name = name.substring(0, name.length-1);
	if (id.length > 0 ) id = id.substring(0, id.length-1);
	$("#prodReturnStatForm").find("#subchannelName").val(name);
	$("#prodReturnStatForm").find("#subchannelId").val(id);
}
...
function onCheckOrderType(e, treeId, treeNode) {
	...
	if (name.length > 0 ) name = name.substring(0, name.length-1);
	if (id.length > 0 ) id = id.substring(0, id.length-1);
	$("#prodReturnStatForm").find("#orderTypeName").val(name);
	$("#prodReturnStatForm").find("#orderTypeId").val(id);
}
...
function onCheckOrderStatus(e, treeId, treeNode) {
	...
	if (name.length > 0 ) name = name.substring(0, name.length-1);
	if (id.length > 0 ) id = id.substring(0, id.length-1);
	$("#prodReturnStatForm").find("#orderStatusName").val(name);
	$("#prodReturnStatForm").find("#orderStatusId").val(id);
}
...
function onCheckReturnReason(e, treeId, treeNode) {
	...
	if (name.length > 0 ) name = name.substring(0, name.length-1);
	if (id.length > 0 ) id = id.substring(0, id.length-1);
	$("#prodReturnStatForm").find("#returnReasonName").val(name);
	$("#prodReturnStatForm").find("#returnReasonId").val(id);
}
...
function ajaxFileUpload()
{
	...
	(
		{
			url:'productStat!upload.action?paramMap.prefix=prodreturnstat_product_',//用于文件上传的服务器端请求地址
...
function ajaxFileUploadUserId()
{
	...
	(
		{
			url:'productStat!uploadUserId.action?paramMap.prefix=prodreturnstat_userid_',//用于文件上传的服务器端请求地址
			secureuri:false,//一般设置为false

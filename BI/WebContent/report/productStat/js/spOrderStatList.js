	...
	$.getCaches("../cache!getCache.action","SEX,EXCLUDERETURNFLAG",function(cacheKeys){
		...
		var options={
				...
				colModel:[
				    ...
				    {label:"一般退货数",name:"returnordercount"},
				    {label:"拒收数",name:"jushoucc"},
...
//		}
		...
		if($("#spOrderStatForm").find("#ageStart").val() != "" && ($("#spOrderStatForm").find("#ageEnd").val() == null || $("#spOrderStatForm").find("#ageEnd").val() == "")){
			$.messager.alert("操作提示", "起始年龄和终止年龄必须同时填写!");
			$("#spOrderStatForm").find("#ageEnd").focus();
			return false;
	 	}
		
		if($("#spOrderStatForm").find("#ageEnd").val() != "" && ($("#spOrderStatForm").find("#ageStart").val() == null || $("#spOrderStatForm").find("#ageStart").val() == "")){
			$.messager.alert("操作提示", "起始年龄和终止年龄必须同时填写!");
			$("#spOrderStatForm").find("#ageStart").focus();
			return false;
	 	}
		...
		if($("#registerEndTime").datebox("getValue") != "" &&($("#registerStartTime").datebox("getValue") == null || $("#registerStartTime").datebox("getValue") == "")){
			...
			$("#spOrderStatForm").find("#registerStartTime").focus();
			return false;
		}
		...
		if($("#actualEndTime").datebox("getValue") != "" &&($("#actualStartTime").datebox("getValue") == null || $("#actualStartTime").datebox("getValue") == "")){
			...
			$("#spOrderStatForm").find("#actualStartTime").focus();
			return false;
		}
		...
		if($("#orderEndTime").datebox("getValue") != "" &&($("#orderStartTime").datebox("getValue") == null || $("#orderStartTime").datebox("getValue") == "")){
			...
			$("#spOrderStatForm").find("#orderStartTime").focus();
			return false;
		}
		...
		jQuery("#spOrderStatTable").jqGrid("setGridParam", {url:"productStat!spOrderStatList.action"}); 
		$("#spOrderStatTable").hlGridRefresh();
	});
	
	$("#resetBtn").click(function(){
		$("#spOrderStatForm").clearForm();
		//会员等级清空
		if($.fn.zTree.getZTreeObj("levelTree") != null){
			$.fn.zTree.getZTreeObj("levelTree").checkAllNodes(false);
		}
		//性别初始化
		$("#spOrderStatForm").find("#sex").val(null);
		$("#spOrderStatForm").find("#excludereturnflag").val("N");
		//会员类型
	...
	$("#clearFileCreatebyid").click(function() {
		...
		$("#clearLevel").click(function() {
			$("#spOrderStatForm").find("#membershiplevelname").val("");
			$("#spOrderStatForm").find("#membershiplevelid").val("");
		...
		$("#clearCustomerType").click(function() {
			$("#spOrderStatForm").find("#customerTypeName").val("");
			$("#spOrderStatForm").find("#customerTypeId").val("");
		...
		$("#clearProductClass").click(function() {
			$("#spOrderStatForm").find("#productClassName").val("");
			$("#spOrderStatForm").find("#productClassId").val("");
		...
		$("#clearSubchannel").click(function() {
			$("#spOrderStatForm").find("#subchannelName").val("");
			$("#spOrderStatForm").find("#subchannelId").val("");
		...
		$("#clearOrderType").click(function() {
			$("#spOrderStatForm").find("#orderTypeName").val("");
			$("#spOrderStatForm").find("#orderTypeId").val("");
		...
		$("#clearOrderStatus").click(function() {
			$("#spOrderStatForm").find("#orderStatusName").val("");
			$("#spOrderStatForm").find("#orderStatusId").val("");
...
function onCheckLvl(e, treeId, treeNode) {
	...
	$("#spOrderStatForm").find("#membershiplevelname").val(name);
	$("#spOrderStatForm").find("#membershiplevelid").val(id);
}
...
function onCheckCustomerType(e, treeId, treeNode) {
	...
	if (name.length > 0 ) name = name.substring(0, name.length-1);
	if (id.length > 0 ) id = id.substring(0, id.length-1);
	$("#spOrderStatForm").find("#customerTypeName").val(name);
	$("#spOrderStatForm").find("#customerTypeId").val(id);
}
...
function onCheckProductClass(e, treeId, treeNode) {
	...
	if (name.length > 0 ) name = name.substring(0, name.length-1);
	if (id.length > 0 ) id = id.substring(0, id.length-1);
	$("#spOrderStatForm").find("#productClassName").val(name);
	$("#spOrderStatForm").find("#productClassId").val(id);
}
...
function onCheckSubchannel(e, treeId, treeNode) {
	...
	if (name.length > 0 ) name = name.substring(0, name.length-1);
	if (id.length > 0 ) id = id.substring(0, id.length-1);
	$("#spOrderStatForm").find("#subchannelName").val(name);
	$("#spOrderStatForm").find("#subchannelId").val(id);
}
...
function onCheckOrderType(e, treeId, treeNode) {
	...
	if (name.length > 0 ) name = name.substring(0, name.length-1);
	if (id.length > 0 ) id = id.substring(0, id.length-1);
	$("#spOrderStatForm").find("#orderTypeName").val(name);
	$("#spOrderStatForm").find("#orderTypeId").val(id);
}
...
function onCheckOrderStatus(e, treeId, treeNode) {
	...
	if (name.length > 0 ) name = name.substring(0, name.length-1);
	if (id.length > 0 ) id = id.substring(0, id.length-1);
	$("#spOrderStatForm").find("#orderStatusName").val(name);
	$("#spOrderStatForm").find("#orderStatusId").val(id);
}
...
function ajaxFileUpload()
{
	...
	(
		{
			url:'productStat!upload.action?paramMap.prefix=sporderstat_product_',//用于文件上传的服务器端请求地址
...
function ajaxFileUploadUserId()
{
	...
	(
		{
			url:'productStat!uploadUserId.action?paramMap.prefix=sporderstat_userid_',//用于文件上传的服务器端请求地址
			secureuri:false,//一般设置为false

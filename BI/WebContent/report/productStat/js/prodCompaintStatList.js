	...
	$.getCaches("../cache!getCache.action","SEX,EXCLUDERETURNFLAG",function(cacheKeys){
		var options={
				...
				colModel:[
				    ...
				    {label:"积分",name:"jifen"},
				    {label:"总订单数",name:"allordercount"},
				    {label:"总退货数",name:"returnordercount"},
				    {label:"客诉主因",name:"compaintreseason"},
				    {label:"客诉订单数",name:"productcount"}
				],
				multiselect:false,
				loadonce: false,
				caption:"查询列表",
				searchform: "#prodCompaintStatForm",
				sortname:"productcount",
				sortorder:"desc",
				allowOper : {add:false,del:false,update:false,exportexcel:true}
			};
		
		$("#prodCompaintStatTable").hlGrid(options);
...
//		}
		...
		if($("#prodCompaintStatForm").find("#ageStart").val() != "" && ($("#prodCompaintStatForm").find("#ageEnd").val() == null || $("#prodCompaintStatForm").find("#ageEnd").val() == "")){
			$.messager.alert("操作提示", "起始年龄和终止年龄必须同时填写!");
			$("#prodCompaintStatForm").find("#ageEnd").focus();
			return false;
	 	}
		
		if($("#prodCompaintStatForm").find("#ageEnd").val() != "" && ($("#prodCompaintStatForm").find("#ageStart").val() == null || $("#prodCompaintStatForm").find("#ageStart").val() == "")){
			$.messager.alert("操作提示", "起始年龄和终止年龄必须同时填写!");
			$("#prodCompaintStatForm").find("#ageStart").focus();
			return false;
	 	}
		...
		if($("#registerEndTime").datebox("getValue") != "" &&($("#registerStartTime").datebox("getValue") == null || $("#registerStartTime").datebox("getValue") == "")){
			...
			$("#prodCompaintStatForm").find("#registerStartTime").focus();
			return false;
		}
		...
		if($("#actualEndTime").datebox("getValue") != "" &&($("#actualStartTime").datebox("getValue") == null || $("#actualStartTime").datebox("getValue") == "")){
			...
			$("#prodCompaintStatForm").find("#actualStartTime").focus();
			return false;
		}
		...
		if($("#orderEndTime").datebox("getValue") != "" &&($("#orderStartTime").datebox("getValue") == null || $("#orderStartTime").datebox("getValue") == "")){
			...
			$("#prodCompaintStatForm").find("#orderStartTime").focus();
			return false;
		}
		...
		jQuery("#prodCompaintStatTable").jqGrid("setGridParam", {url:"productStat!prodCompaintStatList.action"}); 
		$("#prodCompaintStatTable").hlGridRefresh();
	});
	
	$("#resetBtn").click(function(){
		$("#prodCompaintStatForm").clearForm();
		//会员等级清空
		if($.fn.zTree.getZTreeObj("levelTree") != null){
			$.fn.zTree.getZTreeObj("levelTree").checkAllNodes(false);
		}
		//性别初始化
		$("#prodCompaintStatForm").find("#sex").val(null);
		$("#prodCompaintStatForm").find("#excludereturnflag").val("N");
		//会员类型
	...
	$("#clearFileCreatebyid").click(function() {
		...
		$("#clearLevel").click(function() {
			$("#prodCompaintStatForm").find("#membershiplevelname").val("");
			$("#prodCompaintStatForm").find("#membershiplevelid").val("");
		...
		$("#clearCustomerType").click(function() {
			$("#prodCompaintStatForm").find("#customerTypeName").val("");
			$("#prodCompaintStatForm").find("#customerTypeId").val("");
		...
		$("#clearProductClass").click(function() {
			$("#prodCompaintStatForm").find("#productClassName").val("");
			$("#prodCompaintStatForm").find("#productClassId").val("");
		...
		$("#clearSubchannel").click(function() {
			$("#prodCompaintStatForm").find("#subchannelName").val("");
			$("#prodCompaintStatForm").find("#subchannelId").val("");
		...
		$("#clearOrderType").click(function() {
			$("#prodCompaintStatForm").find("#orderTypeName").val("");
			$("#prodCompaintStatForm").find("#orderTypeId").val("");
		...
		$("#clearOrderStatus").click(function() {
			$("#prodCompaintStatForm").find("#orderStatusName").val("");
			$("#prodCompaintStatForm").find("#orderStatusId").val("");
		...
		$("#clearComplaintCategory").click(function() {
			$("#prodCompaintStatForm").find("#complaintCategoryName").val("");
			$("#prodCompaintStatForm").find("#complaintCategoryId").val("");
...
function onCheckLvl(e, treeId, treeNode) {
	...
	$("#prodCompaintStatForm").find("#membershiplevelname").val(name);
	$("#prodCompaintStatForm").find("#membershiplevelid").val(id);
}
...
function onCheckCustomerType(e, treeId, treeNode) {
	...
	if (name.length > 0 ) name = name.substring(0, name.length-1);
	if (id.length > 0 ) id = id.substring(0, id.length-1);
	$("#prodCompaintStatForm").find("#customerTypeName").val(name);
	$("#prodCompaintStatForm").find("#customerTypeId").val(id);
}
...
function onCheckProductClass(e, treeId, treeNode) {
	...
	if (name.length > 0 ) name = name.substring(0, name.length-1);
	if (id.length > 0 ) id = id.substring(0, id.length-1);
	$("#prodCompaintStatForm").find("#productClassName").val(name);
	$("#prodCompaintStatForm").find("#productClassId").val(id);
}
...
function onCheckSubchannel(e, treeId, treeNode) {
	...
	if (name.length > 0 ) name = name.substring(0, name.length-1);
	if (id.length > 0 ) id = id.substring(0, id.length-1);
	$("#prodCompaintStatForm").find("#subchannelName").val(name);
	$("#prodCompaintStatForm").find("#subchannelId").val(id);
}
...
function onCheckOrderType(e, treeId, treeNode) {
	...
	if (name.length > 0 ) name = name.substring(0, name.length-1);
	if (id.length > 0 ) id = id.substring(0, id.length-1);
	$("#prodCompaintStatForm").find("#orderTypeName").val(name);
	$("#prodCompaintStatForm").find("#orderTypeId").val(id);
}
...
function onCheckOrderStatus(e, treeId, treeNode) {
	...
	if (name.length > 0 ) name = name.substring(0, name.length-1);
	if (id.length > 0 ) id = id.substring(0, id.length-1);
	$("#prodCompaintStatForm").find("#orderStatusName").val(name);
	$("#prodCompaintStatForm").find("#orderStatusId").val(id);
}
...
function onCheckComplaintCategory(e, treeId, treeNode) {
	...
	if (name.length > 0 ) name = name.substring(0, name.length-1);
	if (id.length > 0 ) id = id.substring(0, id.length-1);
	$("#prodCompaintStatForm").find("#complaintCategoryName").val(name);
	$("#prodCompaintStatForm").find("#complaintCategoryId").val(id);
}
...
function ajaxFileUpload()
{
	...
	(
		{
			url:'productStat!upload.action?paramMap.prefix=prodcompaintstat_product_',//用于文件上传的服务器端请求地址
...
function ajaxFileUploadUserId()
{
	...
	(
		{
			url:'productStat!uploadUserId.action?paramMap.prefix=prodcompaintstat_userid_',//用于文件上传的服务器端请求地址
			secureuri:false,//一般设置为false

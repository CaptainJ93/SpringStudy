...
<script src="js/mdOrderStatList.js" type="text/javascript"></script>
<style>
...
<body style="overflow-x: hidden;">
	<div class="mainSilde mainBlock">
		...
		<form action="" id="mdOrderStatForm" method="post">
		<%
			String menuName = "";
		...
		<div class="list searchList">
			<ul class="listShow">
			   ...
			   <li>
			   		<label>MD工号:</label>
			   		<input type="text" id="mdid" onkeyup="this.value=this.value.replace(/[^\d!,]/g,'')"/>&nbsp;<span>","分割</span>
			   </li>
			   <li>
			   		<label>MD工号(Excel):</label>
			   		<input type="file" id="fileMDid" name="fileMDid" onchange="ajaxFileUploadMDId()" style="width: 70px"/>
			   		<img src="<%=request.getContextPath()%>/css/img/loading.gif" id="MDidLoading" style="display: none;"/>
			   		<span id="localFileMDName"></span>&nbsp;
			   		<a id="clearFileMDid" href="#">清空</a>&nbsp;<a href="../template/userId.xls">模板</a>
			   		<input type="hidden" id="fileMDName" />
			   </li>
			   <li>
			   		<label>是否排除销退:</label>
		...
		<table id="mdOrderStatTable"></table>
	</div>
</body>
</html>
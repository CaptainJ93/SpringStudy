$(function(){
	$.getCaches("../cache!getCache.action","SEX,EXCLUDERETURNFLAG",function(cacheKeys){
		var options={
				...
				defaultSearch:false,
				colModel:[
				    {label:"姓名",name:"createbyname"},
				    {label:"总订单数",name:"allordercount"},
				    {label:"总订单金额",name:"allorderamout"},
				    {label:"有效订单数",name:"validordercount"},
				    {label:"有效订单金额",name:"validorderamout"},
				    {label:"取消订单金额",name:"cancelorderamount"},
				    {label:"取消订单件数",name:"cancelordercount"},
				...
				loadonce: false,
				caption:"查询列表",
				sortname:"allordercount",
				sortorder:"desc",
				searchform: "#createbyStatForm",
				allowOper : {add:false,del:false,update:false,exportexcel:true}
			};
		
		$("#createbyStatTable").hlGrid(options);
...
//		}
		...
		if($("#createbyStatForm").find("#ageStart").val() != "" && ($("#createbyStatForm").find("#ageEnd").val() == null || $("#createbyStatForm").find("#ageEnd").val() == "")){
			$.messager.alert("操作提示", "起始年龄和终止年龄必须同时填写!");
			$("#createbyStatForm").find("#ageEnd").focus();
			return false;
	 	}
		
		if($("#createbyStatForm").find("#ageEnd").val() != "" && ($("#createbyStatForm").find("#ageStart").val() == null || $("#createbyStatForm").find("#ageStart").val() == "")){
			$.messager.alert("操作提示", "起始年龄和终止年龄必须同时填写!");
			$("#createbyStatForm").find("#ageStart").focus();
			return false;
	 	}
		...
		if($("#registerEndTime").datebox("getValue") != "" &&($("#registerStartTime").datebox("getValue") == null || $("#registerStartTime").datebox("getValue") == "")){
			...
			$("#createbyStatForm").find("#registerStartTime").focus();
			return false;
		}
		...
		if($("#actualEndTime").datebox("getValue") != "" &&($("#actualStartTime").datebox("getValue") == null || $("#actualStartTime").datebox("getValue") == "")){
			...
			$("#createbyStatForm").find("#actualStartTime").focus();
			return false;
		}
		...
		if($("#orderEndTime").datebox("getValue") != "" &&($("#orderStartTime").datebox("getValue") == null || $("#orderStartTime").datebox("getValue") == "")){
			...
			$("#createbyStatForm").find("#orderStartTime").focus();
			return false;
		}
		...
		jQuery("#createbyStatTable").jqGrid("setGridParam", {url:"assessStat!createbyStatList.action"}); 
		$("#createbyStatTable").hlGridRefresh();
	});
	
	$("#resetBtn").click(function(){
		$("#createbyStatForm").clearForm();
		//会员等级清空
		$.fn.zTree.getZTreeObj("levelTree").checkAllNodes(false);
		//性别初始化
		$("#createbyStatForm").find("#sex").val(null);
		$("#createbyStatForm").find("#excludereturnflag").val("N");
		//会员类型
	...
	$("#clearFileCreatebyid").click(function() {
		...
		$("#clearLevel").click(function() {
			$("#createbyStatForm").find("#membershiplevelname").val("");
			$("#createbyStatForm").find("#membershiplevelid").val("");
		...
		$("#clearCustomerType").click(function() {
			$("#createbyStatForm").find("#customerTypeName").val("");
			$("#createbyStatForm").find("#customerTypeId").val("");
		...
		$("#clearProductClass").click(function() {
			$("#createbyStatForm").find("#productClassName").val("");
			$("#createbyStatForm").find("#productClassId").val("");
		...
		$("#clearSubchannel").click(function() {
			$("#createbyStatForm").find("#subchannelName").val("");
			$("#createbyStatForm").find("#subchannelId").val("");
		...
		$("#clearOrderType").click(function() {
			$("#createbyStatForm").find("#orderTypeName").val("");
			$("#createbyStatForm").find("#orderTypeId").val("");
		...
		$("#clearOrderStatus").click(function() {
			$("#createbyStatForm").find("#orderStatusName").val("");
			$("#createbyStatForm").find("#orderStatusId").val("");
...
function onCheckLvl(e, treeId, treeNode) {
	...
	$("#createbyStatForm").find("#membershiplevelname").val(name);
	$("#createbyStatForm").find("#membershiplevelid").val(id);
}
...
function onCheckCustomerType(e, treeId, treeNode) {
	...
	if (name.length > 0 ) name = name.substring(0, name.length-1);
	if (id.length > 0 ) id = id.substring(0, id.length-1);
	$("#createbyStatForm").find("#customerTypeName").val(name);
	$("#createbyStatForm").find("#customerTypeId").val(id);
}
...
function onCheckProductClass(e, treeId, treeNode) {
	...
	if (name.length > 0 ) name = name.substring(0, name.length-1);
	if (id.length > 0 ) id = id.substring(0, id.length-1);
	$("#createbyStatForm").find("#productClassName").val(name);
	$("#createbyStatForm").find("#productClassId").val(id);
}
...
function onCheckSubchannel(e, treeId, treeNode) {
	...
	if (name.length > 0 ) name = name.substring(0, name.length-1);
	if (id.length > 0 ) id = id.substring(0, id.length-1);
	$("#createbyStatForm").find("#subchannelName").val(name);
	$("#createbyStatForm").find("#subchannelId").val(id);
}
...
function onCheckOrderType(e, treeId, treeNode) {
	...
	if (name.length > 0 ) name = name.substring(0, name.length-1);
	if (id.length > 0 ) id = id.substring(0, id.length-1);
	$("#createbyStatForm").find("#orderTypeName").val(name);
	$("#createbyStatForm").find("#orderTypeId").val(id);
}
...
function onCheckOrderStatus(e, treeId, treeNode) {
	...
	if (name.length > 0 ) name = name.substring(0, name.length-1);
	if (id.length > 0 ) id = id.substring(0, id.length-1);
	$("#createbyStatForm").find("#orderStatusName").val(name);
	$("#createbyStatForm").find("#orderStatusId").val(id);
}
...
function ajaxFileUpload()
{
	...
	(
		{
			url:'assessStat!upload.action?paramMap.prefix=createbystat_product_',//用于文件上传的服务器端请求地址
...
function ajaxFileUploadUserId()
{
	...
	(
		{
			url:'assessStat!uploadUserId.action?paramMap.prefix=createbystat_userid_',//用于文件上传的服务器端请求地址
			secureuri:false,//一般设置为false

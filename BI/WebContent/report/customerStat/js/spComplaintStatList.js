...
$.getCaches("../cache!getCache.action","",function(cacheKeys){
	var options={
			...
			sortorder:"desc",
			searchform: "#spComplaintStatForm",
			allowOper : {add:false,del:false,update:false,exportexcel:true}
		};
	$("#spComplaintStatTable").hlGrid(options);
...
//		}
		...
		if($("#orderEndTime").datebox("getValue") != "" &&($("#orderStartTime").datebox("getValue") == null || $("#orderStartTime").datebox("getValue") == "")){
			...
			$("#spComplaintStatForm").find("#orderStartTime").focus();
			return false;
		}
		...
		jQuery("#spComplaintStatTable").jqGrid("setGridParam", {url:"customerStat!spComplaintStatList.action"}); 
		$("#spComplaintStatTable").hlGridRefresh();
	});
	
	$("#resetBtn").click(function(){
		$("#spComplaintStatForm").clearForm();
		//会员等级清空
	...
	$("#clearFileHandlebyid").click(function() {
		...
		$("#clearLevel").click(function() {
			$("#spComplaintStatForm").find("#membershiplevelname").val("");
			$("#spComplaintStatForm").find("#membershiplevelid").val("");
		...
		$("#clearCustomerType").click(function() {
			$("#spComplaintStatForm").find("#customerTypeName").val("");
			$("#spComplaintStatForm").find("#customerTypeId").val("");
		...
		$("#clearReturnReason").click(function() {
			$("#spComplaintStatForm").find("#returnReasonName").val("");
			$("#spComplaintStatForm").find("#returnReasonId").val("");
		...
		$("#clearComplaintCategory").click(function() {
			$("#spComplaintStatForm").find("#complaintCategoryName").val("");
			$("#spComplaintStatForm").find("#complaintCategoryId").val("");
...
function onCheckLvl(e, treeId, treeNode) {
	...
	$("#spComplaintStatForm").find("#membershiplevelname").val(name);
	$("#spComplaintStatForm").find("#membershiplevelid").val(id);
}
...
function onCheckCustomerType(e, treeId, treeNode) {
	...
	if (name.length > 0 ) name = name.substring(0, name.length-1);
	if (id.length > 0 ) id = id.substring(0, id.length-1);
	$("#spComplaintStatForm").find("#customerTypeName").val(name);
	$("#spComplaintStatForm").find("#customerTypeId").val(id);
}
...
function onCheckReturnReason(e, treeId, treeNode) {
	...
	if (name.length > 0 ) name = name.substring(0, name.length-1);
	if (id.length > 0 ) id = id.substring(0, id.length-1);
	$("#spComplaintStatForm").find("#returnReasonName").val(name);
	$("#spComplaintStatForm").find("#returnReasonId").val(id);
}
...
function onCheckComplaintCategory(e, treeId, treeNode) {
	...
	if (name.length > 0 ) name = name.substring(0, name.length-1);
	if (id.length > 0 ) id = id.substring(0, id.length-1);
	$("#spComplaintStatForm").find("#complaintCategoryName").val(name);
	$("#spComplaintStatForm").find("#complaintCategoryId").val(id);
}
...
function ajaxFileUpload()
{
	...
	(
		{
			url:'customerStat!upload.action?paramMap.prefix=spComplaintStat_product_',//用于文件上传的服务器端请求地址
...
function ajaxFileUploadUserId()
{
	...
	(
		{
			url:'customerStat!uploadUserId.action?paramMap.prefix=spComplaintStat_userid_',//用于文件上传的服务器端请求地址
			secureuri:false,//一般设置为false
...
function ajaxFileUploadUserId2()
{
	...
	(
		{
			url:'customerStat!uploadHandleUserId.action?paramMap.prefix=spComplaintStat_userid2_',//用于文件上传的服务器端请求地址
			secureuri:false,//一般设置为false

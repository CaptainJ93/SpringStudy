$(function(){
	$.getCaches("../cache!getCache.action","SEX,EXCLUDERETURNFLAG",function(cacheKeys){
		var options={
				...
				sortorder:"desc",
				searchform: "#sourceChannelCustStatForm",
				allowOper : {add:false,del:false,update:false,exportexcel:true}
			};
		$("#sourceChannelCustStatTable").hlGrid(options);
...
//		}
		...
		if($("#sourceChannelCustStatForm").find("#ageStart").val() != "" && ($("#sourceChannelCustStatForm").find("#ageEnd").val() == null || $("#sourceChannelCustStatForm").find("#ageEnd").val() == "")){
			$.messager.alert("操作提示", "起始年龄和终止年龄必须同时填写!");
			$("#sourceChannelCustStatForm").find("#ageEnd").focus();
			return false;
	 	}
		
		if($("#sourceChannelCustStatForm").find("#ageEnd").val() != "" && ($("#sourceChannelCustStatForm").find("#ageStart").val() == null || $("#sourceChannelCustStatForm").find("#ageStart").val() == "")){
			$.messager.alert("操作提示", "起始年龄和终止年龄必须同时填写!");
			$("#sourceChannelCustStatForm").find("#ageStart").focus();
			return false;
	 	}
		...
		if($("#registerEndTime").datebox("getValue") != "" &&($("#registerStartTime").datebox("getValue") == null || $("#registerStartTime").datebox("getValue") == "")){
			...
			$("#sourceChannelCustStatForm").find("#registerStartTime").focus();
			return false;
		}
		...
		if($("#actualEndTime").datebox("getValue") != "" &&($("#actualStartTime").datebox("getValue") == null || $("#actualStartTime").datebox("getValue") == "")){
			...
			$("#sourceChannelCustStatForm").find("#actualStartTime").focus();
			return false;
		}
		...
		if($("#orderEndTime").datebox("getValue") != "" &&($("#orderStartTime").datebox("getValue") == null || $("#orderStartTime").datebox("getValue") == "")){
			...
			$("#sourceChannelCustStatForm").find("#orderStartTime").focus();
			return false;
		}
		...
		jQuery("#sourceChannelCustStatTable").jqGrid("setGridParam", {url:"customerStat!sourceChannelCustStatList.action"}); 
		$("#sourceChannelCustStatTable").hlGridRefresh();
	});
	
	$("#resetBtn").click(function(){
		$("#sourceChannelCustStatForm").clearForm();
		//会员等级清空
		if($.fn.zTree.getZTreeObj("levelTree") != null){
			$.fn.zTree.getZTreeObj("levelTree").checkAllNodes(false);
		}
		//性别初始化
		$("#sourceChannelCustStatForm").find("#sex").val(null);
		$("#sourceChannelCustStatForm").find("#excludereturnflag").val("N");
		//会员类型
	...
	$("#clearFileCreatebyid").click(function() {
		...
		$("#clearLevel").click(function() {
			$("#sourceChannelCustStatForm").find("#membershiplevelname").val("");
			$("#sourceChannelCustStatForm").find("#membershiplevelid").val("");
		...
		$("#clearCustomerType").click(function() {
			$("#sourceChannelCustStatForm").find("#customerTypeName").val("");
			$("#sourceChannelCustStatForm").find("#customerTypeId").val("");
		...
		$("#clearProductClass").click(function() {
			$("#sourceChannelCustStatForm").find("#productClassName").val("");
			$("#sourceChannelCustStatForm").find("#productClassId").val("");
		...
		$("#clearSubchannel").click(function() {
			$("#sourceChannelCustStatForm").find("#subchannelName").val("");
			$("#sourceChannelCustStatForm").find("#subchannelId").val("");
		...
		$("#clearOrderType").click(function() {
			$("#sourceChannelCustStatForm").find("#orderTypeName").val("");
			$("#sourceChannelCustStatForm").find("#orderTypeId").val("");
		...
		$("#clearOrderStatus").click(function() {
			$("#sourceChannelCustStatForm").find("#orderStatusName").val("");
			$("#sourceChannelCustStatForm").find("#orderStatusId").val("");
...
function onCheckLvl(e, treeId, treeNode) {
	...
	$("#sourceChannelCustStatForm").find("#membershiplevelname").val(name);
	$("#sourceChannelCustStatForm").find("#membershiplevelid").val(id);
}
...
function onCheckCustomerType(e, treeId, treeNode) {
	...
	if (name.length > 0 ) name = name.substring(0, name.length-1);
	if (id.length > 0 ) id = id.substring(0, id.length-1);
	$("#sourceChannelCustStatForm").find("#customerTypeName").val(name);
	$("#sourceChannelCustStatForm").find("#customerTypeId").val(id);
}
...
function onCheckProductClass(e, treeId, treeNode) {
	...
	if (name.length > 0 ) name = name.substring(0, name.length-1);
	if (id.length > 0 ) id = id.substring(0, id.length-1);
	$("#sourceChannelCustStatForm").find("#productClassName").val(name);
	$("#sourceChannelCustStatForm").find("#productClassId").val(id);
}
...
function onCheckSubchannel(e, treeId, treeNode) {
	...
	if (name.length > 0 ) name = name.substring(0, name.length-1);
	if (id.length > 0 ) id = id.substring(0, id.length-1);
	$("#sourceChannelCustStatForm").find("#subchannelName").val(name);
	$("#sourceChannelCustStatForm").find("#subchannelId").val(id);
}
...
function onCheckOrderType(e, treeId, treeNode) {
	...
	if (name.length > 0 ) name = name.substring(0, name.length-1);
	if (id.length > 0 ) id = id.substring(0, id.length-1);
	$("#sourceChannelCustStatForm").find("#orderTypeName").val(name);
	$("#sourceChannelCustStatForm").find("#orderTypeId").val(id);
}
...
function onCheckOrderStatus(e, treeId, treeNode) {
	...
	if (name.length > 0 ) name = name.substring(0, name.length-1);
	if (id.length > 0 ) id = id.substring(0, id.length-1);
	$("#sourceChannelCustStatForm").find("#orderStatusName").val(name);
	$("#sourceChannelCustStatForm").find("#orderStatusId").val(id);
}
...
function ajaxFileUpload()
{
	...
	(
		{
			url:'customerStat!upload.action?paramMap.prefix=sourceChannelCustStat_product_',//用于文件上传的服务器端请求地址
...
function ajaxFileUploadUserId()
{
	...
	(
		{
			url:'customerStat!uploadUserId.action?paramMap.prefix=sourceChannelCustStat_userid_',//用于文件上传的服务器端请求地址
			secureuri:false,//一般设置为false

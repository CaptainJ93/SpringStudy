$(function(){
	$.getCaches("../cache!getCache.action","SEX,EXCLUDERETURNFLAG",function(cacheKeys){
		var options={
				...
				sortorder:"desc",
				searchform: "#channelCustStatForm",
				allowOper : {add:false,del:false,update:false,exportexcel:true}
			};
		$("#channelCustStatTable").hlGrid(options);
...
//		}
		...
		if($("#channelCustStatForm").find("#ageStart").val() != "" && ($("#channelCustStatForm").find("#ageEnd").val() == null || $("#channelCustStatForm").find("#ageEnd").val() == "")){
			$.messager.alert("操作提示", "起始年龄和终止年龄必须同时填写!");
			$("#channelCustStatForm").find("#ageEnd").focus();
			return false;
	 	}
		
		if($("#channelCustStatForm").find("#ageEnd").val() != "" && ($("#channelCustStatForm").find("#ageStart").val() == null || $("#channelCustStatForm").find("#ageStart").val() == "")){
			$.messager.alert("操作提示", "起始年龄和终止年龄必须同时填写!");
			$("#channelCustStatForm").find("#ageStart").focus();
			return false;
	 	}
		...
		if($("#registerEndTime").datebox("getValue") != "" &&($("#registerStartTime").datebox("getValue") == null || $("#registerStartTime").datebox("getValue") == "")){
			...
			$("#channelCustStatForm").find("#registerStartTime").focus();
			return false;
		}
		...
		if($("#actualEndTime").datebox("getValue") != "" &&($("#actualStartTime").datebox("getValue") == null || $("#actualStartTime").datebox("getValue") == "")){
			...
			$("#channelCustStatForm").find("#actualStartTime").focus();
			return false;
		}
		...
		if($("#orderEndTime").datebox("getValue") != "" &&($("#orderStartTime").datebox("getValue") == null || $("#orderStartTime").datebox("getValue") == "")){
			...
			$("#channelCustStatForm").find("#orderStartTime").focus();
			return false;
		}
		...
		jQuery("#channelCustStatTable").jqGrid("setGridParam", {url:"customerStat!channelCustStatList.action"}); 
		$("#channelCustStatTable").hlGridRefresh();
	});
	
	$("#resetBtn").click(function(){
		$("#channelCustStatForm").clearForm();
		//会员等级清空
		if($.fn.zTree.getZTreeObj("levelTree") != null){
			$.fn.zTree.getZTreeObj("levelTree").checkAllNodes(false);
		}
		//性别初始化
		$("#channelCustStatForm").find("#sex").val(null);
		$("#channelCustStatForm").find("#excludereturnflag").val("N");
		//会员类型
	...
	$("#clearFileCreatebyid").click(function() {
		...
		$("#clearLevel").click(function() {
			$("#channelCustStatForm").find("#membershiplevelname").val("");
			$("#channelCustStatForm").find("#membershiplevelid").val("");
		...
		$("#clearCustomerType").click(function() {
			$("#channelCustStatForm").find("#customerTypeName").val("");
			$("#channelCustStatForm").find("#customerTypeId").val("");
		...
		$("#clearProductClass").click(function() {
			$("#channelCustStatForm").find("#productClassName").val("");
			$("#channelCustStatForm").find("#productClassId").val("");
		...
		$("#clearSubchannel").click(function() {
			$("#channelCustStatForm").find("#subchannelName").val("");
			$("#channelCustStatForm").find("#subchannelId").val("");
		...
		$("#clearOrderType").click(function() {
			$("#channelCustStatForm").find("#orderTypeName").val("");
			$("#channelCustStatForm").find("#orderTypeId").val("");
		...
		$("#clearOrderStatus").click(function() {
			$("#channelCustStatForm").find("#orderStatusName").val("");
			$("#channelCustStatForm").find("#orderStatusId").val("");
...
function onCheckLvl(e, treeId, treeNode) {
	...
	$("#channelCustStatForm").find("#membershiplevelname").val(name);
	$("#channelCustStatForm").find("#membershiplevelid").val(id);
}
...
function onCheckCustomerType(e, treeId, treeNode) {
	...
	if (name.length > 0 ) name = name.substring(0, name.length-1);
	if (id.length > 0 ) id = id.substring(0, id.length-1);
	$("#channelCustStatForm").find("#customerTypeName").val(name);
	$("#channelCustStatForm").find("#customerTypeId").val(id);
}
...
function onCheckProductClass(e, treeId, treeNode) {
	...
	if (name.length > 0 ) name = name.substring(0, name.length-1);
	if (id.length > 0 ) id = id.substring(0, id.length-1);
	$("#channelCustStatForm").find("#productClassName").val(name);
	$("#channelCustStatForm").find("#productClassId").val(id);
}
...
function onCheckSubchannel(e, treeId, treeNode) {
	...
	if (name.length > 0 ) name = name.substring(0, name.length-1);
	if (id.length > 0 ) id = id.substring(0, id.length-1);
	$("#channelCustStatForm").find("#subchannelName").val(name);
	$("#channelCustStatForm").find("#subchannelId").val(id);
}
...
function onCheckOrderType(e, treeId, treeNode) {
	...
	if (name.length > 0 ) name = name.substring(0, name.length-1);
	if (id.length > 0 ) id = id.substring(0, id.length-1);
	$("#channelCustStatForm").find("#orderTypeName").val(name);
	$("#channelCustStatForm").find("#orderTypeId").val(id);
}
...
function onCheckOrderStatus(e, treeId, treeNode) {
	...
	if (name.length > 0 ) name = name.substring(0, name.length-1);
	if (id.length > 0 ) id = id.substring(0, id.length-1);
	$("#channelCustStatForm").find("#orderStatusName").val(name);
	$("#channelCustStatForm").find("#orderStatusId").val(id);
}
...
function ajaxFileUpload()
{
	...
	(
		{
			url:'customerStat!upload.action?paramMap.prefix=channelCustStat_product_',//用于文件上传的服务器端请求地址
...
function ajaxFileUploadUserId()
{
	...
	(
		{
			url:'customerStat!uploadUserId.action?paramMap.prefix=channelCustStat_userid_',//用于文件上传的服务器端请求地址
			secureuri:false,//一般设置为false

$(function(){
	$.getCaches("../cache!getCache.action","SEX,EXCLUDERETURNFLAG",function(cacheKeys){
		...
		$("#gendarCustStatTable").hlGrid(options);
...
//		}
		...
		if($("#gendarCustStatForm").find("#ageStart").val() != "" && ($("#gendarCustStatForm").find("#ageEnd").val() == null || $("#gendarCustStatForm").find("#ageEnd").val() == "")){
			$.messager.alert("操作提示", "起始年龄和终止年龄必须同时填写!");
			$("#gendarCustStatForm").find("#ageEnd").focus();
			return false;
	 	}
		
		if($("#gendarCustStatForm").find("#ageEnd").val() != "" && ($("#gendarCustStatForm").find("#ageStart").val() == null || $("#gendarCustStatForm").find("#ageStart").val() == "")){
			$.messager.alert("操作提示", "起始年龄和终止年龄必须同时填写!");
			$("#gendarCustStatForm").find("#ageStart").focus();
			return false;
	 	}
		...
		if($("#registerEndTime").datebox("getValue") != "" &&($("#registerStartTime").datebox("getValue") == null || $("#registerStartTime").datebox("getValue") == "")){
			...
			$("#gendarCustStatForm").find("#registerStartTime").focus();
			return false;
		}
		...
		if($("#actualEndTime").datebox("getValue") != "" &&($("#actualStartTime").datebox("getValue") == null || $("#actualStartTime").datebox("getValue") == "")){
			...
			$("#gendarCustStatForm").find("#actualStartTime").focus();
			return false;
		}
		...
		if($("#orderEndTime").datebox("getValue") != "" &&($("#orderStartTime").datebox("getValue") == null || $("#orderStartTime").datebox("getValue") == "")){
			...
			$("#gendarCustStatForm").find("#orderStartTime").focus();
			return false;
		}
		...
		jQuery("#gendarCustStatTable").jqGrid("setGridParam", {url:"customerStat!gendarCustStatList.action"}); 
		$("#gendarCustStatTable").hlGridRefresh();
	});
	
	$("#resetBtn").click(function(){
		$("#gendarCustStatForm").clearForm();
		//会员等级清空
		if($.fn.zTree.getZTreeObj("levelTree") != null){
			$.fn.zTree.getZTreeObj("levelTree").checkAllNodes(false);
		}
		//性别初始化
		$("#gendarCustStatForm").find("#sex").val(null);
		$("#gendarCustStatForm").find("#excludereturnflag").val("N");
		//会员类型
	...
	$("#clearFileCreatebyid").click(function() {
		...
		$("#clearLevel").click(function() {
			$("#gendarCustStatForm").find("#membershiplevelname").val("");
			$("#gendarCustStatForm").find("#membershiplevelid").val("");
		...
		$("#clearCustomerType").click(function() {
			$("#gendarCustStatForm").find("#customerTypeName").val("");
			$("#gendarCustStatForm").find("#customerTypeId").val("");
		...
		$("#clearProductClass").click(function() {
			$("#gendarCustStatForm").find("#productClassName").val("");
			$("#gendarCustStatForm").find("#productClassId").val("");
		...
		$("#clearSubchannel").click(function() {
			$("#gendarCustStatForm").find("#subchannelName").val("");
			$("#gendarCustStatForm").find("#subchannelId").val("");
		...
		$("#clearOrderType").click(function() {
			$("#gendarCustStatForm").find("#orderTypeName").val("");
			$("#gendarCustStatForm").find("#orderTypeId").val("");
		...
		$("#clearOrderStatus").click(function() {
			$("#gendarCustStatForm").find("#orderStatusName").val("");
			$("#gendarCustStatForm").find("#orderStatusId").val("");
...
function onCheckLvl(e, treeId, treeNode) {
	...
	$("#gendarCustStatForm").find("#membershiplevelname").val(name);
	$("#gendarCustStatForm").find("#membershiplevelid").val(id);
}
...
function onCheckCustomerType(e, treeId, treeNode) {
	...
	if (name.length > 0 ) name = name.substring(0, name.length-1);
	if (id.length > 0 ) id = id.substring(0, id.length-1);
	$("#gendarCustStatForm").find("#customerTypeName").val(name);
	$("#gendarCustStatForm").find("#customerTypeId").val(id);
}
...
function onCheckProductClass(e, treeId, treeNode) {
	...
	if (name.length > 0 ) name = name.substring(0, name.length-1);
	if (id.length > 0 ) id = id.substring(0, id.length-1);
	$("#gendarCustStatForm").find("#productClassName").val(name);
	$("#gendarCustStatForm").find("#productClassId").val(id);
}
...
function onCheckSubchannel(e, treeId, treeNode) {
	...
	if (name.length > 0 ) name = name.substring(0, name.length-1);
	if (id.length > 0 ) id = id.substring(0, id.length-1);
	$("#gendarCustStatForm").find("#subchannelName").val(name);
	$("#gendarCustStatForm").find("#subchannelId").val(id);
}
...
function onCheckOrderType(e, treeId, treeNode) {
	...
	if (name.length > 0 ) name = name.substring(0, name.length-1);
	if (id.length > 0 ) id = id.substring(0, id.length-1);
	$("#gendarCustStatForm").find("#orderTypeName").val(name);
	$("#gendarCustStatForm").find("#orderTypeId").val(id);
}
...
function onCheckOrderStatus(e, treeId, treeNode) {
	...
	if (name.length > 0 ) name = name.substring(0, name.length-1);
	if (id.length > 0 ) id = id.substring(0, id.length-1);
	$("#gendarCustStatForm").find("#orderStatusName").val(name);
	$("#gendarCustStatForm").find("#orderStatusId").val(id);
}
...
function ajaxFileUpload()
{
	...
	(
		{
			url:'customerStat!upload.action?paramMap.prefix=gendarCustStat_product_',//用于文件上传的服务器端请求地址
...
function ajaxFileUploadUserId()
{
	...
	(
		{
			url:'customerStat!uploadUserId.action?paramMap.prefix=gendarCustStat_userid_',//用于文件上传的服务器端请求地址
			secureuri:false,//一般设置为false
...
function drawFusionChart(xml){
	if(getChartFromId("chartLevle")){
		...
		if(xml == null || xml.gendarCustStatChart == null || xml.gendarCustStatChart == ""){
			chartReference.setDataXML("");
		}else{
			chartReference.setDataXML(xml.gendarCustStatChart);
		}
	}else{
		...
		if(xml == null || xml.gendarCustStatChart == null || xml.gendarCustStatChart == ""){
			...
			chart1.setDataXML(xml.gendarCustStatChart);
		}

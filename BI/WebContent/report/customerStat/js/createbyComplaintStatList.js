$(function(){
	...
	var options={
			...
			defaultSearch:false,
			colModel:[
			    {label:"建立人",name:"createdbyname"},
			    {label:"后抛客诉量",name:"assignedcompquantity"}
			],
			multiselect:false,
			loadonce: false,
			caption:"查询列表",
			sortname:"assignedcompquantity",
			sortorder:"desc",
			searchform: "#createbyComplaintStatForm",
			allowOper : {add:false,del:false,update:false,exportexcel:true}
		};
	$("#createbyComplaintStatTable").hlGrid(options);
...
//		}
		...
		if($("#orderEndTime").datebox("getValue") != "" &&($("#orderStartTime").datebox("getValue") == null || $("#orderStartTime").datebox("getValue") == "")){
			...
			$("#createbyComplaintStatForm").find("#orderStartTime").focus();
			return false;
		}
		...
		jQuery("#createbyComplaintStatTable").jqGrid("setGridParam", {url:"customerStat!createbyComplaintStatList.action"}); 
		$("#createbyComplaintStatTable").hlGridRefresh();
	});
	
	$("#resetBtn").click(function(){
		$("#createbyComplaintStatForm").clearForm();
		//会员等级清空
	...
	$("#clearFileHandlebyid").click(function() {
		...
		$("#clearLevel").click(function() {
			$("#createbyComplaintStatForm").find("#membershiplevelname").val("");
			$("#createbyComplaintStatForm").find("#membershiplevelid").val("");
		...
		$("#clearCustomerType").click(function() {
			$("#createbyComplaintStatForm").find("#customerTypeName").val("");
			$("#createbyComplaintStatForm").find("#customerTypeId").val("");
		...
		$("#clearReturnReason").click(function() {
			$("#createbyComplaintStatForm").find("#returnReasonName").val("");
			$("#createbyComplaintStatForm").find("#returnReasonId").val("");
		...
		$("#clearComplaintCategory").click(function() {
			$("#createbyComplaintStatForm").find("#complaintCategoryName").val("");
			$("#createbyComplaintStatForm").find("#complaintCategoryId").val("");
...
function onCheckLvl(e, treeId, treeNode) {
	...
	$("#createbyComplaintStatForm").find("#membershiplevelname").val(name);
	$("#createbyComplaintStatForm").find("#membershiplevelid").val(id);
}
...
function onCheckCustomerType(e, treeId, treeNode) {
	...
	if (name.length > 0 ) name = name.substring(0, name.length-1);
	if (id.length > 0 ) id = id.substring(0, id.length-1);
	$("#createbyComplaintStatForm").find("#customerTypeName").val(name);
	$("#createbyComplaintStatForm").find("#customerTypeId").val(id);
}
...
function onCheckReturnReason(e, treeId, treeNode) {
	...
	if (name.length > 0 ) name = name.substring(0, name.length-1);
	if (id.length > 0 ) id = id.substring(0, id.length-1);
	$("#createbyComplaintStatForm").find("#returnReasonName").val(name);
	$("#createbyComplaintStatForm").find("#returnReasonId").val(id);
}
...
function onCheckComplaintCategory(e, treeId, treeNode) {
	...
	if (name.length > 0 ) name = name.substring(0, name.length-1);
	if (id.length > 0 ) id = id.substring(0, id.length-1);
	$("#createbyComplaintStatForm").find("#complaintCategoryName").val(name);
	$("#createbyComplaintStatForm").find("#complaintCategoryId").val(id);
}
...
function ajaxFileUpload()
{
	...
	(
		{
			url:'customerStat!upload.action?paramMap.prefix=createbyComplaintStat_product_',//用于文件上传的服务器端请求地址
...
function ajaxFileUploadUserId()
{
	...
	(
		{
			url:'customerStat!uploadUserId.action?paramMap.prefix=createbyComplaintStat_userid_',//用于文件上传的服务器端请求地址
			secureuri:false,//一般设置为false
...
function ajaxFileUploadUserId2()
{
	...
	(
		{
			url:'customerStat!uploadHandleUserId.action?paramMap.prefix=createbyComplaintStat_userid2_',//用于文件上传的服务器端请求地址
			secureuri:false,//一般设置为false

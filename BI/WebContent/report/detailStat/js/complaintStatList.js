$(function(){
	...
	var options={
			...
			exporturl:"detailStat!exportComplaintStat.action",//用于数据导出url
			defaultSearch:false,
			colModel:[
			    ...
			    {label:"客诉编号",name:"complaintid",width:"100",fixed:true},
			    {label:"订单号",name:"orderid",width:"100",fixed:true},
			    {label:"订单建立时间",name:"ordereddate",width:"150",fixed:true},
			    {label:"销退单号",name:"returnid",width:"100",fixed:true},
			    {label:"销退建立时间",name:"returndate",width:"150",fixed:true},
			    {label:"品号",name:"productid",width:"100",fixed:true},
			    {label:"品名",name:"productname",width:"200",fixed:true},
			    {label:"厂商名",name:"suppliername",width:"200",fixed:true},
			    {label:"地址一",name:"address1",width:"200",fixed:true},
			    {label:"客诉建立日期",name:"complainteddate",width:"150",fixed:true},
			    {label:"客诉主因",name:"complaintcategoryname",width:"200",fixed:true},
			    {label:"客诉内容",name:"complaintconnect",width:"200",fixed:true},
			    {label:"客诉建立人",name:"complaintedbyname",width:"100",fixed:true},
			    {label:"客诉结案日期",name:"closeddate",width:"150",fixed:true},
			    {label:"客诉处理人",name:"assignedtoname",width:"100",fixed:true},
			    {label:"客诉意见",name:"comments",width:"150",fixed:true},
			    {label:"快递公司",name:"carriername",width:"100",fixed:true},
			    {label:"快递号",name:"deliveryorderid",width:"200",fixed:true}
			],
			multiselect:false,
			loadonce: false,
			caption:"查询列表",
		...
		$("#complaintStatTable").hlGrid(options);
		$("#complaintStatTable").closest(".ui-jqgrid-bdiv").css({ 'overflow-x' : 'hidden' });
	});
...
//		}
		...
		jQuery("#complaintStatTable").jqGrid("setGridParam", {url:"detailStat!complaintStatList.action"}); 
		$("#complaintStatTable").hlGridRefresh();
		$("#complaintStatTable").closest(".ui-jqgrid-bdiv").css({ 'overflow-x' : 'scroll' });
	});
	
	$("#resetBtn").click(function(){
		$("#complaintStatForm").clearForm();
		//会员等级清空
	...
	$("#clearFileHandlebyid").click(function() {
		...
		$("#clearLevel").click(function() {
			$("#complaintStatForm").find("#membershiplevelname").val("");
			$("#complaintStatForm").find("#membershiplevelid").val("");
		...
		$("#clearCustomerType").click(function() {
			$("#complaintStatForm").find("#customerTypeName").val("");
			$("#complaintStatForm").find("#customerTypeId").val("");
		...
		$("#clearReturnReason").click(function() {
			$("#complaintStatForm").find("#returnReasonName").val("");
			$("#complaintStatForm").find("#returnReasonId").val("");
		...
		$("#clearComplaintCategory").click(function() {
			$("#complaintStatForm").find("#complaintCategoryName").val("");
			$("#complaintStatForm").find("#complaintCategoryId").val("");
...
function onCheckLvl(e, treeId, treeNode) {
	...
	$("#complaintStatForm").find("#membershiplevelname").val(name);
	$("#complaintStatForm").find("#membershiplevelid").val(id);
}
...
function onCheckCustomerType(e, treeId, treeNode) {
	...
	if (name.length > 0 ) name = name.substring(0, name.length-1);
	if (id.length > 0 ) id = id.substring(0, id.length-1);
	$("#complaintStatForm").find("#customerTypeName").val(name);
	$("#complaintStatForm").find("#customerTypeId").val(id);
}
...
function onCheckReturnReason(e, treeId, treeNode) {
	...
	if (name.length > 0 ) name = name.substring(0, name.length-1);
	if (id.length > 0 ) id = id.substring(0, id.length-1);
	$("#complaintStatForm").find("#returnReasonName").val(name);
	$("#complaintStatForm").find("#returnReasonId").val(id);
}
...
function onCheckComplaintCategory(e, treeId, treeNode) {
	...
	if (name.length > 0 ) name = name.substring(0, name.length-1);
	if (id.length > 0 ) id = id.substring(0, id.length-1);
	$("#complaintStatForm").find("#complaintCategoryName").val(name);
	$("#complaintStatForm").find("#complaintCategoryId").val(id);
}
...
function ajaxFileUpload()
{
	...
	(
		{
			url:'detailStat!upload.action?paramMap.prefix=complaintStat_product_',//用于文件上传的服务器端请求地址
...
function ajaxFileUploadUserId()
{
	...
	(
		{
			url:'detailStat!uploadUserId.action?paramMap.prefix=complaintStat_userid_',//用于文件上传的服务器端请求地址
			secureuri:false,//一般设置为false
...
function ajaxFileUploadUserId2()
{
	...
	(
		{
			url:'detailStat!uploadHandleUserId.action?paramMap.prefix=complaintStat_userid2_',//用于文件上传的服务器端请求地址
			secureuri:false,//一般设置为false

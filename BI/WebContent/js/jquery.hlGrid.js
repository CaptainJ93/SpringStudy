/*
主要功能：对jqGrid再进行一层封装，简化参数的设置
1 方便的增删改查操作，都只需要写一句代码
2 与后台方便的集成，提交的参数直接进入dto/hashmap，简化action中的代码
3 数据字典转换功能，支持直接从缓存获取字典数据
4 表格随页面大小的变化，自动适应
5 将很多常用参数设为默认，简化使用
*/
...
a.p.serializeGridData.call(a,a.p.postData):a.p.postData,success:function(s){if (s!=undefined&&s["errCode"]!=undefined&&s["errCode"]==-1) {if (s["errMsg"]!=undefined) alert("存储过程返回错误: "+s.errMsg);else alert("存储过程返回错误");return;}/*add by cf*/if(chartFunction!=null)eval(chartFunction);/*add by cf*/o==="xml"?ea(s,a.grid.bDiv,G,d>1,C):L(s,a.grid.bDiv,G,d>1,C);x&&x.call(a,s);g&&a.grid.populateVisible();if(a.p.loadonce||a.p.treeGrid)a.p.datatype="local";oa()},error:function(s,v,y){if(s.status == '303' || s.status == '0'/*IE*/){top.location.reload();}/*add by cf*/b.isFunction(a.p.loadError)&&a.p.loadError.call(a,s,v,y);oa()},beforeSend:function(s){ra();b.isFunction(a.p.loadBeforeSend)&&a.p.loadBeforeSend.call(a,s)}},b.jgrid.ajaxOptions,a.p.ajaxGridOptions));break;case "xmlstring":ra();h=b.jgrid.stringToDoc(a.p.datastr);
...
function(D){if(this.hidden===false&&!this.fixed){this.width=x=Math.round(C*this.width/(a.p.tblwidth-G));d+=x;o=D}});y=0;if(w){if(e.width-s-(d+g*h)!==p)y=e.width-s-(d+g*h)-p}else if(!w&&Math.abs(e.width-s-(d+g*h))!==1)y=e.width-s-(d+g*h);a.p.colModel[o].width+=y-15;a.p.tblwidth=d+y+G+v*g;if(a.p.tblwidth>a.p.width){a.p.colModel[o].width-=a.p.tblwidth-parseInt(a.p.width,10);a.p.tblwidth=a.p.width}}})();b(J).css("width",e.width+"px").append("<div class='ui-jqgrid-resize-mark' id='rs_m"+a.p.id+"'>&#160;</div>");
...
"ui-subgrid"&&b(e).removeClass("ui-state-highlight").attr("aria-selected","false");e=false;b("#jqg_"+c.p.id+"_"+b.jgrid.jqID(c.p.selrow)).attr("checked",e);c.p.selarrrow.splice(k,1);c.p.onSelectRow&&j&&c.p.onSelectRow.call(c,c.p.selrow,e);e=c.p.selarrrow[0];c.p.selrow=e===undefined?null:e}}else if(e.className!=="ui-subgrid"){c.p.selrow&&b(c.rows.namedItem(c.p.selrow)).removeClass("ui-state-highlight").attr("aria-selected","false");if(c.p.selrow!=e.id){
b("#jqg_"+c.p.id+"_"+b.jgrid.jqID(c.p.selrow)).attr("checked",false);b("#jqg_"+c.p.id+"_"+b.jgrid.jqID(e.id)).attr("checked",true);//ylw
...
this,a=-1;if(n.grid){if(isNaN(f))b(n.p.colModel).each(function(F){if(this.name===f){a=F;return false}});else a=parseInt(f,10);if(a>=0){var r=n.rows.length,t=1;if(r&&r>0){for(;t<r;){if(b(n.rows[t]).hasClass("jqgrow")){try{e=b.unformat(b(n.rows[t].cells[a]),{rowId:n.rows[t].id,colModel:n.p.colModel[a]},a)}catch(B){e=b.jgrid.htmlDecode(n.rows[t].cells[a].innerHTML)}
//if(e==undefined||e==null)continue;//add by ylw
if(i)k+=parseFloat(e);else if(j)c.push({id:n.rows[t].id,value:e});else c.push(e)}t++}if(i)switch(i.toLowerCase()){case "sum":c=k;break;case "avg":c=
...
this.isValue(a))return false;else if(!this.isValue(a))return true;a=c.trim(a).replace(/\&nbsp\;/ig,"").replace(/\&#160\;/ig,"");return a===""}});c.fn.fmatter=function(a,b,d,f,e){alert(JSON.stringify(f));var i=b;d=c.extend({},c.jgrid.formatter,d);if(c.fn.fmatter[a])i=c.fn.fmatter[a](b,d,f,e);return i};c.fmatter.util={NumberFormat:function(a,b){c.fmatter.isNumber(a)||(a*=1);if(c.fmatter.isNumber(a)){var d=a<0,f=a+"",e=b.decimalSeparator?b.decimalSeparator:".";if(c.fmatter.isNumber(b.decimalPlaces)){var i=b.decimalPlaces;f=
...
;(function($) {
	...
	$.fn.fmatter.link = function(cellval, opts) {
		...
		if(!$.fmatter.isEmpty(cellval)) {
			return "<a "+target+" href=\"" + cellval + "\" style=\"text-decoration:underline\">" + cellval + "</a>";
		}else {
			return $.fn.fmatter.defaultFormat(cellval,opts);
		}
	};
	$.fn.fmatter.showlink = function(cellval, opts, rwd) {
		...
		if(typeof op.addParam=="object") {
			...
			$.each(op.addParam,function(k,v){
				if (rwd[v]!=undefined) addParam[k]=rwd[v]; else addParam[k]=v;
			});
			op.addParam="&"+$.param(addParam);
		}
		idUrl = op.baseLinkUrl+op.showAction + '?'+ op.idName+'='+opts.rowId+op.addParam;
		if($.fmatter.isString(cellval) || $.fmatter.isNumber(cellval)) {	//add this one even if its blank string
			return "<a "+target+" href=\"" + idUrl + "\" style=\"text-decoration:underline\">" + cellval + "</a>";
		}else {
			return $.fn.fmatter.defaultFormat(cellval,opts);
		}
	};
...
// 基础函数 
// 为map中key，添加一个前缀 
function addMapKeyPrefix(prefix,map){
	var np={};
	for (var k in map){
		np[prefix+k]=map[k];
	};
...
var chartFunction;
jQuery.fn.extend({
	hlGrid: function(opts) {

	var gridId="#"+this.attr("id");
	var rbId="jqg_"+this.attr("id");
	var pagerId="";
	if (opts.pager==undefined) {
		pagerId=this.attr("id")+"-pager";
		this.after("<div id=\""+pagerId+"\"></div>");
	}
	// 默认开启autowidth 兼容ie6 
	if (opts["autowidth"]==undefined||opts["autowidth"]) {
		// 设置jqgrid的autowidth为false，改为自己控制 
		opts["autowidth"]=false;
		var gridAutoId=this.attr("id")+"-auto";
		opts["gridAutoId"]=gridAutoId;
		this.after("<div id=\""+gridAutoId+"\" style=\"width:100%;height:0px;position:relative;z-index:-999\"></div>");
		opts["width"]=$("#"+gridAutoId).width()-5;
		if ($.browser.msie) {
			$("#"+gridAutoId).resize(function(){
				$(gridId).hlGridAdjustWidth();
			});
		} else {
	...
	// 自定义参数初始化 
	var dto="paramMap";
	if (opts["dto"]==undefined) opts["dto"]=dto;
	if (opts["editdto"]==undefined) opts["editdto"]=opts["dto"];
	if (opts["deldto"]==undefined) opts["deldto"]=opts["dto"]; // 删除默认用查询的dto 
	if (opts["adddto"]==undefined) opts["adddto"]=opts["editdto"];

	if (opts["dto"]!="") opts["dto"]+=".";
	if (opts["editdto"]!="") opts["editdto"]+=".";
	if (opts["deldto"]!="") opts["deldto"]+=".";
	if (opts["adddto"]!="") opts["adddto"]+=".";

	if (opts["editurl"]==undefined || opts["editurl"]=="") opts["editurl"]=opts["url"];
	if (opts["delurl"]==undefined) opts["delurl"]=opts["editurl"];
	if (opts["addurl"]==undefined) opts["addurl"]=opts["editurl"];
	
	if (opts["updateform"]!=undefined && opts["editform"]==undefined) opts["editform"]=opts["updateform"];
	
	opts["firstgridcomplete"]=false;
	opts["dictObj"]={};
	if (opts["allowOper"]==undefined) opts["allowOper"]={add:true,del:true,update:true,exportexcel:false,exportxml:false,exportjson:false};
	
	if (opts["chartFunction"]!=undefined) chartFunction = opts["chartFunction"];
	
	//begin add by 
	var beforeSend = function(){return true;};
	if(opts["beforeSend"]==undefined||typeof opts["beforeSend"]!="function") opts["beforeSend"]=beforeSend;
	if(opts["beforeAddSend"]==undefined||typeof opts["beforeAddSend"]!="function") opts["beforeAddSend"]=opts["beforeSend"];
	if(opts["beforeUpdateSend"]==undefined||typeof opts["beforeUpdateSend"]!="function") opts["beforeUpdateSend"]=opts["beforeSend"];
	if(opts["beforeDeleteSend"]==undefined||typeof opts["beforeDeleteSend"]!="function") opts["beforeDeleteSend"]=opts["beforeSend"];
	if(opts["beforeSearchSend"]==undefined||typeof opts["beforeSearchSend"]!="function") opts["beforeSearchSend"]=opts["beforeSend"];
	//end add by 
	
	//将字段名从驼峰转成下划线
	function convertColName(str) {
		var arr=[];
		for(var a=0;a<str.length;a++){
			var ascii=str.charCodeAt(a);
			if (ascii>=65 && ascii<=90) {
				if (arr.length>0)	arr.push("_");
				arr.push(String.fromCharCode(ascii+32));
			} else arr.push(String.fromCharCode(ascii));
		}
		return arr.join("");
	}
	$.each(opts.colModel, function(i,col) {
		if (opts["keyname"]==undefined||opts["keyname"]=="") opts["keyname"]=col["name"];
		if (col["key"]!=undefined && col["key"]==true) opts["keyname"]=col["name"];
		if (col["index"]==undefined) col["index"]=convertColName(col["name"]);
		
		// 转换数据字典
		if (typeof col["codedataobject"]=="object") {
			var codedescname=col["name"];
			var value = "codeFlag";
			var key = "codeName";
			
			if(col["valueField"]!=undefined){
				value = col["valueField"];
			}
			
			if(col["textField"]!=undefined){
				key = col["textField"];
			}
			
			opts["dictObj"][codedescname]=$.dictv2h(value,key,col["codedataobject"]);
			$(gridId).jqGrid("setGridParam",{dictObj:opts["dictObj"]});
		} else {
			if (col["codename"]&&col["codedatasource"]) {
				$.getJSON(col["codedatasource"],{nd:(new Date).getTime()},function(data){
					var codename=col["codename"];
					var codedescname=col["name"];
					opts["dictObj"][codedescname]=$.dictv2h("codeFlag","codeName",data);
					
					$(gridId).jqGrid("setGridParam",{dictObj:opts["dictObj"]});
					// 如果表格数据已经完成加载，则需要立即对code字段进行翻译 
					if ($(gridId).jqGrid("getGridParam","firstgridcomplete")){
						$(gridId).hlSetGridData(function(rowid,row){
							var fv=row[codename];
							var d=opts["dictObj"][codedescname];
							row[codedescname]=d[fv];
						});
					}
				});
			}
	...
	// 扩展参数 
	if (opts["allowOper"]&&(opts["allowOper"]["update"]||opts["allowOper"]["del"])) {
		var colw=0;
		if (opts["allowOper"]["update"]) colw+=50;
		if (opts["allowOper"]["del"]) colw+=50;
		opts["colModel"].push({label:"操作",name:"GridAction",width:colw,editable:false,sortable:false});
		if (opts["colNames"]) opts["colNames"].push("操作");
	}
	if (opts["singleselect"]){
		opts["multiselect"]=false;
		opts["colModel"].unshift({label:" ",name:"rb",width:28,editable:false,sortable:false});
		if (opts["colNames"]) opts["colNames"].unshift("");
		
		//var sfunc=opts["onSelectRow"];
		//opts["onSelectRow"]=function (rowid){
		//	$(gridId).find(gridId+"-rb-"+rowid).attr("checked",true);
		//	if (typeof sfunc=="function") sfunc(rowid);
		//}
	}
	var gcf=opts["gridComplete"];
	opts["gridComplete"]=function()
	{
		$(gridId).jqGrid("setGridParam",{firstgridcomplete:true});
		var ao=$(gridId).jqGrid("getGridParam","allowOper");
		var cols=$(gridId).jqGrid("getGridParam","colModel");
		var dos=$(gridId).jqGrid("getGridParam","dictObj");
		var ss=$(gridId).jqGrid("getGridParam","singleselect");

		var isfirst = opts["isFirstSelect"]==undefined ? true : opts["isFirstSelect"];
		
		$(gridId).hlSetGridData(function(rowid,row){
			row["GridAction"]="";
			//rd["GridAction"]="<table cellspacing=\"0\" cellpadding=\"0\" border=\"0\" class=\"ui-pg-table navtable\" style=\"float:left;table-layout:auto;\" ><tbody><tr><td class=\"ui-pg-button ui-corner-all\"><div class=\"ui-pg-div\"><span class=\"ui-icon ui-icon-pencil\" title=\"编辑记录\" style=\"cursor:pointer\" onclick=\"$('"+gridId+"').hlGridUpdatePre("+cl+")\"></span></div></td><td class=\"ui-pg-button ui-corner-all\"><span class=\"ui-icon ui-icon-trash\" title=\"删除记录\" style=\"cursor:pointer\" onclick=\"$('"+gridId+"').hlGridDelete('"+cl+"')\"></span></td><tr></tbody></table>";
			if (ao&&ao["update"])
				row["GridAction"]+="<a title=\"编辑记录\" style=\"cursor:pointer\" onclick=\"javascript:$('"+gridId+"').hlGridUpdatePre("+rowid+")\">编辑</a>&nbsp;";// 更改手型样式为cursor，cursor支持多浏览器，hand在谷歌下不支持
			if (ao&&ao["del"])
				row["GridAction"]+="<a title=\"删除记录\" style=\"cursor:pointer\" onclick=\"javascript:$('"+gridId+"').hlGridDelete("+rowid+")\">删除</a>";//更改手型样式为cursor，cursor支持多浏览器，hand在谷歌下不支持
			if (ss) {
				if (isfirst){
					$(gridId).jqGrid("setSelection",rowid,false);
					row["rb"]="<input type=\"radio\" id=\""+rbId+"_"+rowid+"\" checked=\"true\" onclick=\"$('"+gridId+"').jqGrid('setSelection','"+rowid+"',true)\"/>";
					isfirst=false;
				} else row["rb"]="<input type=\"radio\" id=\""+rbId+"_"+rowid+"\" onclick=\"$('"+gridId+"').jqGrid('setSelection','"+rowid+"',true)\"/>";
			}
			
			$.each(cols,function(i,c) {
				if (c["codename"]) {
					if (dos[c["name"]])
						row[c["name"]]=dos[c["name"]][row[c["codename"]]];
				}
			});
		});
		if (typeof gcf=="function") gcf();
	};
	...
	if (opts["inGridEdit"]) {
		...
		{
			if (rowid) {
				if (lastsel>0 && rowid!==lastsel) {
					$(gridId).jqGrid("saveRow",lastsel);
					$(gridId).jqGrid("setRowData",lastsel,{GridAction:""});
				}
				$(gridId).jqGrid('setRowData',rowid,{GridAction: "<input style='height:22px;width:50px;' type='button' value='保存' onclick=\"$('"+gridId+"').saveRow('"+rowid+"'); $('"+gridId+"').setRowData('"+rowid+"',{GridAction:''});\" />"});
	...
	defaults={
		caption:"hlGrid",
		pager:"#"+pagerId,	// 显示页码的div 
		datatype:"json",		// 数据源格式  
		//mtype:"get",				// 提交方式 get 或者 post
		mtype:"post",	//用get会产生乱码，modify by cf
		rowNum:20,         	// 每页默认显示记录数  
		rowList:[10,20,50],	// 每页显示记录数可选列表  
		viewrecords:true,  	// 显示右下角的总记录数 
		height: "auto",    	// 高度自适应 
		multiselect:true,   // 默认多选 
		jsonReader:{repeatitems:false}, // 设定数据源格式 
		prmNames:{page:opts["dto"]+"page",rows:opts["dto"]+"rows",sort:opts["dto"]+"sidx",order:opts["dto"]+"sord",search:"_search",nd:"nd",id:opts["keyname"],oper:opts["dto"]+"oper",subgridid:opts["dto"]+"subid",npage:null,totalrows:opts["dto"]+"totalrows"}// 设置提交服务器的参数名称，默认为paramMap.xxx, baseAction中有setParamMap方法，参数可自动进入paramMap
		
		//begin 
		,defaultBeforeAddSend:function(e,xhr){
			var $flag = opts["beforeAddSend"](e,xhr);
			if($flag!=undefined&&$flag!=true){
				return false;
			}
			return $(opts["addform"]).validatebox("isAllValid");//调用扩展的方法来校验页面easyui的validatebox元素是否填值正确
		},
		defaultBeforeUpdateSend:function(e,xhr){
			var $flag = opts["beforeUpdateSend"](e,xhr);
			if($flag!=undefined&&$flag!=true){
				return false;
			}
			return $(opts["editform"]).validatebox("isAllValid");//调用扩展的方法来校验页面easyui的validatebox元素是否填值正确
		},
		defaultBeforeDeleteSend:function(e,xhr){
			var $flag = opts["beforeDeleteSend"](e,xhr);
			if($flag!=undefined&&$flag!=true){
				return false;
			}
			return true;
		},
		defaultBeforeSearchSend:function(obj){
			var $flag = opts["beforeSearchSend"](obj);
			if($flag!=undefined&&$flag!=true){
				return false;
			}
			return $(opts["searchform"]).validatebox("isAllValid");//调用扩展的方法来校验页面easyui的validatebox元素是否填值正确
		}
		//end
	}
	// 设置默认参数 
	...
	if (opts.pager!=="") {
		this.navGrid(opts["pager"],{edit:false,add:false,del:false,search:false,refresh:false});
		if (opts["allowOper"]) {
			if (opts["allowOper"]["refresh"])
			...
			if (opts["allowOper"]["add"])
				this.navButtonAdd(opts["pager"],{
					caption:"新增",
					buttonicon:"ui-icon-plus",
					onClickButton: function(){
						func=$(gridId).jqGrid("getGridParam","onAdd");
						if (typeof func=="function") func();
					},
					position:"last"
				});
			if (opts["allowOper"]["del"])
				this.navButtonAdd(opts["pager"],{
					caption:"删除",
			...
			if (opts["allowOper"]["exportexcel"])
				this.navButtonAdd(opts["pager"],{
					caption:"导出Excel",
					buttonicon:"ui-icon-document",
					onClickButton: function(){
						//$(gridId).hlGridExportExcel();
						$(gridId).hlGridExportExcelChoose();
					},
					position:"last"
				});
		}
	...
	hlSetGridData: function(func){
		if (func==undefined||typeof func!="function") return;
		var rowids = this.jqGrid("getDataIDs");
		for(var i in rowids) {
			var row=this.jqGrid("getRowData",rowids[i]);			func(rowids[i], row);
			this.jqGrid("setRowData",rowids[i],row);
		}
	},
	hlGridTranslate: function(rowid){
		var dictObj=this.jqGrid("getGridParam","dictObj");
		var cols=this.jqGrid("getGridParam","colModel");
		var row=this.jqGrid("getRowData",rowid);
		$.each(cols,function(i,c) {
			if (c["codename"]) {
				if (dictObj[c["name"]])
					row[c["name"]]=dictObj[c["name"]][row[c["codename"]]];
			}
		});
		this.jqGrid("setRowData",rowid,row);
	},
	hlGridRefresh: function(opts){
		if (opts==undefined || opts["postData"]==undefined) {
			var sfrm=$(this.jqGrid('getGridParam','searchform'));
			var pdata={};
			if (sfrm!=undefined) {
				pdata=sfrm.unbindData();
				pdata=addMapKeyPrefix(this.jqGrid('getGridParam','dto'),pdata);
			}
		} else {
			var pdata=opts["postData"];
		}
		
		//begin
		if(!this.jqGrid("getGridParam","defaultBeforeSearchSend")(pdata)){
			return false;
		}
		//end
		this.jqGrid("setGridParam",{postData:{"paramMap.isCilckRefreshBtn":true}});//点击搜索按钮时才去执行存储过程
		if (opts==undefined || opts["url"]==undefined || opts["url"]=="")
			this.jqGrid("setGridParam",{datatype:"json",postData:pdata,defaultSearch:true}).trigger('reloadGrid',[{page:1}]);
		else
			this.jqGrid("setGridParam",{url:opts["url"],datatype:"json",postData:pdata,defaultSearch:true}).trigger('reloadGrid',[{page:1}]);
		this.jqGrid("setGridParam",{postData:{"paramMap.isCilckRefreshBtn":false}});//点击搜索按钮时才去执行存储过程
	},
	hlGridDelete: function(rowid) {
		if (rowid==undefined) {
			var rowid=[];
			$.merge(rowid, this.jqGrid('getGridParam','selarrrow'));
			if (!rowid.length) rowid=this.jqGrid('getGridParam','selrow');
		}
		...
		var dfunc=this.jqGrid("getGridParam","onDelete");
		if (typeof dfunc=="function") if(dfunc(rowid)==false) return;

		var sfrm=$(this.jqGrid('getGridParam','delform'));
		var pdata={};
		if (sfrm!=undefined) {
			pdata=sfrm.unbindData();
			pdata=addMapKeyPrefix(this.jqGrid('getGridParam','deldto'),pdata);
		}
		pdata["oper"]="del";
		pdata[this.jqGrid("getGridParam","deldto")+"oper"]="del";
		if (typeof rowid=="object") {
			// 删除多行 
			var jq=this;
			// 考虑到删除多条记录的时候，需要提交多个id（用，分割），必须用hashmap 
			pdata[this.jqGrid("getGridParam","deldto")+this.jqGrid("getGridParam","keyname")]=rowid.join(",");
		} else {
			// 删除一行 
			pdata[this.jqGrid("getGridParam","deldto")+this.jqGrid("getGridParam","keyname")]=rowid;
		}
		
		var vurl = this.jqGrid("getGridParam","delurl");
		if( vurl == undefined || vurl=="" ){
			if (typeof rowid=="object") {
				for (var i in rowid) {
					jq.jqGrid("delRowData",rowid[i]);
				}
			} else this.jqGrid("delRowData", rowid);
		}else{
			jQuery.ajax({
				type: "POST",
				url: this.jqGrid("getGridParam","delurl"),
				data: pdata,
				context:this,
				...
				success: function (data,textStatus) {
					...
					var func=this.jqGrid("getGridParam","onDeleteSuccess");
					if (typeof func=="function") func(rowid);
					*/
					this.hlGridRefresh();
				...
				dataType: "json"
				//begin 
				,beforeSend: this.jqGrid("getGridParam","defaultBeforeDeleteSend")
				//end 
			});	
		}
	},
	hlGridUpdatePre: function(rowid) {
		if (rowid==undefined) {
			var arrrow=this.jqGrid("getGridParam","selarrrow");
			if (arrrow.length>1) {
				$.messager.alert("操作提示", "您选中了多条记录, 编辑时只能选择一条记录, 请确认!");
				return;
			}
			if (arrrow.length==0) {
				var selrow=this.jqGrid("getGridParam","selrow");
				if (selrow==null || selrow==undefined) {
					$.messager.alert("操作提示", "您没有选中任何记录, 请选中一条记录再点编辑!");
					return;
				}
				arrrow.push(selrow);
			}
		...
		var ufrm=$(this.jqGrid("getGridParam","editform"));
		var ufunc=this.jqGrid("getGridParam","onUpdate");
		if (ufrm==undefined && typeof ufunc!="function") {
			alert("未指定updateform或者事件处理函数 ");
			return;
		}
		
		if (typeof ufunc=="function") ufunc(rowdata);
		if (ufrm) ufrm.bindData(rowdata);
	},
	hlGridUpdate: function(postData) {
		var rowid=this.jqGrid("getGridParam","selrow");
		if (postData==undefined) {
			var ufrm=$(this.jqGrid("getGridParam","editform"));
			if (typeof ufrm!="object") {
				alert("未指定 updateform");
				return;
			}
			var pdata=ufrm.unbindData();
		} else pdata=postData;
		
		pdata["oper"]="edit";
		//var pdata1=addMapKeyPrefix(this.jqGrid('getGridParam','editdto'),pdata);
		//pdata1["oper"]="edit";
		
		jQuery.ajax({
			type: "POST",
			url: this.jqGrid("getGridParam","editurl"),
			//data: pdata1,	modify by cf
			data:pdata,
			context:this,
			error: function(XMLHttpRequest,textStatus){
				if(XMLHttpRequest.status == '303' || XMLHttpRequest.status == '12150'){
			...
			success: function (data,textStatus) {
				...
				/*
				this.jqGrid("setRowData",rowid,pdata);
				this.hlGridTranslate(rowid);
				var func=this.jqGrid("getGridParam","onUpdateSuccess");
				if (typeof func=="function") func(pdata);
				*/
				this.hlGridRefresh();
				$.messager.alert("操作结果", "操作成功");
			},
			dataType: "json"
			//begin 
			,beforeSend: this.jqGrid("getGridParam","defaultBeforeUpdateSend")
			//end 
		});
	},
	hlGridAdd: function(opts) {
		var pdata=opts;
		//var pdata1=opts;//
		if (typeof pdata!="object") {
			var afrm=$(this.jqGrid("getGridParam","addform"));
			if (typeof afrm!="object") {
				alert("未指定 addform");
				return;
			}
			var pdata=afrm.unbindData();
		}else var pdata=opts;//
		pdata["oper"]="add";
		pdata1=addMapKeyPrefix(this.jqGrid('getGridParam','adddto'),pdata);
		pdata1["oper"]="add";
		//}//
		jQuery.ajax({
			type: "POST",
			url: this.jqGrid("getGridParam","addurl"),
			//data: pdata1,  modify by cf
			data: pdata,
			context:this,
			error: function(XMLHttpRequest,textStatus){
			...
			success: function (data,textStatus) {
				...
				var retData={};
				if(data!=undefined && data["rows"]!=undefined && $.isArray(data["rows"])){
					retData=data.rows[0];
				}
				if(data!=undefined && data["resultMap"]!=undefined){
					retData=data.resultMap;
				}
				$.extend(pdata,retData);
				/* 
				 * modify by cf
				 * 
				var rowid=new Date().getTime();
				if (pdata[this.jqGrid("getGridParam","keyname")]!=undefined) rowid=pdata[this.jqGrid("getGridParam","keyname")];
				this.jqGrid("addRowData",rowid,pdata,"first");
				this.hlGridTranslate(rowid);
				if (this.jqGrid("getGridParam","refreshafteradd"))	this.hlGridRefresh();
				
				var func=this.jqGrid("getGridParam","onAddSuccess");
				if (typeof func=="function") func(pdata);
				*/
				this.hlGridRefresh();
				$.messager.alert("操作结果", "操作成功");
			...
			dataType: "json"
			//begin 
			,beforeSend: this.jqGrid("getGridParam","defaultBeforeAddSend")
			//end 
		});
	},
	hlGridAdjustWidth: function(){
		$("#gview_"+this.attr("id")).addClass("ui-jqgrid");
		$("#"+this.attr("id")+"-pager").addClass("ui-jqgrid");
		var gridAuto="#"+this.jqGrid("getGridParam","gridAutoId");
		var currGridWidth=$(gridAuto).width();
		var lastGridWidth=this.jqGrid("getGridParam","width");
		if (Math.abs(lastGridWidth-currGridWidth)>5) {
			this.jqGrid("setGridWidth",$(gridAuto).width()-5,true);
		}
	},
	hlGridExportExcelChoose: function(opts){
		if(this.jqGrid("getGridParam","exporturl")!=null&&this.jqGrid("getGridParam","exporturl")!=""){
			this.hlGridExportExcel2(opts);
		}else{
			this.hlGridExportExcel1(opts);
		}
	},
	/*最初的excel数据导出*/
	hlGridExportExcel1: function(opts){
		var url=this.jqGrid("getGridParam","url");
		var cols=this.jqGrid("getGridParam","colModel");
		var labels=[]; var names=[];
		$.each(cols,function(n,v){
			if(v.hidden||v.name=="rb"||v.name=="cb") return;
			labels.push(v.label);
			names.push(v.name);
		});
		if (opts==undefined) {
			var sfrm=$(this.jqGrid('getGridParam','searchform'));
			var pdata={};
//			if (sfrm!=undefined) pdata=sfrm.unbindData();//当界面信息量比较大的时候无法导出到本地，暂时先不传这些信息
			pdata["rows"]=1000000;
			pdata["page"]=1;
			pdata["export"]="excel";
			pdata["colLabels"]=labels.join(",");
			pdata["colnames"]=names.join(",");
			pdata=addMapKeyPrefix(this.jqGrid('getGridParam','dto'),pdata);
		} else {
			var pdata=opts["postData"]==undefined?opts:opts["postData"];
			pdata["rows"]=1000000;
			pdata["page"]=1;
			pdata["export"]="excel";
			pdata["colLabels"]=labels.join(",");
		...
		pdata["nd"]=(new Date()).getTime();
		self.location.href=url.indexOf('?')<0 ? url+"?"+$.param(pdata) : url+"&"+$.param(pdata);
	},
	/*变更后的excel数据导出，主要用于对敏感数据、大数据的excel导出*/
	hlGridExportExcel2: function(opts){
		showLoading();
		var url=this.jqGrid("getGridParam","exporturl");
		if(url==null||url == ''){
			alert("未配置数据导出路径！");
			return false;
		}
		...
		if (opts==undefined) {
			...
			var pdata={};
			var ppdata={};
			if (sfrm!=undefined) ppdata=sfrm.unbindData();//当界面信息量比较大的时候无法导出到本地，暂时先不传这些信息
			pdata["sensitiveData"]=ppdata["sensitiveData"];//从界面上获取是否为敏感数据的判断
			pdata["menuName"]=ppdata["menuName"];//从界面上获取功能所属菜单名
			pdata["rows"]=1000000;
			pdata["page"]=1;
			pdata["export"]="excel";
			pdata["colLabels"]=labels.join(",");
...
//		self.location.href=url.indexOf('?')<0 ? url+"?"+$.param(pdata) : url+"&"+$.param(pdata);
		/*
		 * 修改原因：
		 * 导出数据前需判断导出的数据是否超过系统限制
		 */
		jQuery.ajax({
			type: "POST",
			url: url.indexOf('?')<0 ? url+"?"+$.param(pdata) : url+"&"+$.param(pdata),
			data:{"paramMap.exportAction":"0"},//0表示先查询导出的数据量
			context:this,
			dataType: "json",
			success: function (data,textStatus) {
				if (data!=undefined&&data["errCode"]!=0) {
					...
					if (data["errMsg"]!=undefined)
						...
						$.messager.alert("操作提示","服务器返回错误");
					return;
				}else{
					self.location.href=url.indexOf('?')<0 ? url+"?"+$.param(pdata) : url+"&"+$.param(pdata);
					/*
					 *本地导出excel时根据数据量大小移除刷新div 
					 */
					if(data["value"] <= 20000){
						setTimeout(function(){removeLoading()},3000);
					}else if(data["value"] > 20000 && data["value"] <= 30000){
						setTimeout(function(){removeLoading()},4000);
					}else if(data["value"] > 30000 && data["value"] <= 40000){
						setTimeout(function(){removeLoading()},5000);
					}else if(data["value"] > 40000 && data["value"] <= 50000){
						setTimeout(function(){removeLoading()},6000);
					}else if(data["value"] > 50000 && data["value"] <= 60000){
						setTimeout(function(){removeLoading()},7000);
					}else{
	...
	//支持常见报表格式：只包含一行表头，不支持分页
	freezGrid:function(opts){
		var table = $("#"+this.attr("id"));
		var tableId = this.attr("id");
		
		var sfrm=$(opts["searchform"]);
		var pdata={};
		if (sfrm!=undefined) {
			pdata=sfrm.unbindData();
			pdata=addMapKeyPrefix("paramMap.",pdata);
		}
		
		if(opts["postData"] != undefined){
			jQuery.extend(pdata, opts["postData"]); 
		}
		...
		jQuery.ajax({
			...
			success:function(data,status){
					...
					for(var i in opts["colModel"]){
						result = result + "<th align='center' width='"+opts["colModel"][i]["width"]+"'>"+opts["colModel"][i]["label"]+"</th>";
					}
					result = result + "</tr>";
					
					for(var i in data["rows"]){
						result = result + "<tr>";
						for(var j in opts["colModel"]){
							var colModelName = opts["colModel"][j]["name"];
							result = result + "<td width='"+opts["colModel"][j]["width"]+"'>"+eval("data['rows'][i]['"+colModelName+"']")+"</td>";
						}
						result = result + "</tr>";
				      }
					table.empty();
					...
					if (typeof(freezeRowNum) != 'undefined' || typeof(freezeColumnNum) != 'undefined') {
						freezeTable(table, freezeRowNum || 0, freezeColumnNum || 0, pageWidth(), pageHeight()-$(opts["searchform"]).get(0).offsetHeight-15);
						var flag = false;
						$(window).resize(function() {
							if (flag) 
								return ;
